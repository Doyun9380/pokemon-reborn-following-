...
ItemHandlers::UseOnPokemon.copy(:FIRESTONE,
   ...
   :ASTRALSTONE,:VARIASTONE,:ABYSSALCORE,:DIVINESHARD)
...
ItemHandlers::UseOnPokemon.add(:MAXMUSHROOMS,proc{|item,pokemon,scene|
  canequip=false
  unless pokemon.item==PBItems::MAXMUSHROOMS2
    canequip=true
  end
  if canequip
    scene.pbDisplay(_INTL("The {1} will be given to {2} so that it can use its Z-Power!",PBItems.getName(item),pokemon.name))
...
ItemHandlers::UseOnPokemon.add(:DNASPLICERS,proc{|item,pokemon,scene|
   ...
   elsif (pokemon.species == PBSpecies::ESPEON) && pokemon.hp>=0
     if pokemon.fused!=nil
       if $Trainer.party.length>=6
         scene.pbDisplay(_INTL("Your party is full! You can't unfuse {1}.",pokemon.name))
         next false
       ...
       if chosen>=0
         poke2=$Trainer.party[chosen]
         if (isConst?(poke2.species,PBSpecies,:GLACEON) ||
            isConst?(poke2.species,PBSpecies,:UMBREON)) && poke2.hp>=0
           pokemon.form=2 if isConst?(poke2.species,PBSpecies,:GLACEON)
           pokemon.form=3 if isConst?(poke2.species,PBSpecies,:UMBREON)
           pokemon.fused=poke2
           pbRemovePokemonAt(chosen)
           scene.pbHardRefresh
           scene.pbDisplay(_INTL("{1} changed Forme!",pokemon.name))
...
ItemHandlers::UseOnPokemon.add(:DNAFUSERS,proc{|item,pokemon,scene|
  #=============================================================================
  # 설정
  #=============================================================================
  REQUIRED_POKEMON_COUNT = 3
  
  # 합체 조합 정의: [선택된 3마리 포켓몬의 Species ID 배열] => 합체 결과 Species ID
  # 주의: 배열의 ID는 오름차순으로 정렬해서 키(Key)로 사용해야 합니다.
  FUSION_RECIPES = {
    # 예시 조합 1: A, B, C를 합치면 BULBMANTLE이 나옴
    [PBSpecies::BULBASAUR, PBSpecies::CHARMANDER, PBSpecies::SQUIRTLE] => PBSpecies::BULBMANTLE,
    # 예시 조합 2: D, E, F를 합치면 NEWFUSEDMON이 나옴 (사용자 정의 상수)
    [PBSpecies::CHIKORITA, PBSpecies::CYNDAQUIL, PBSpecies::TOTODILE] => PBSpecies::TORITAQUIL, 
    # 추가적인 합체 레시피를 여기에 계속 정의할 수 있습니다.
	[PBSpecies::TREECKO, PBSpecies::TORCHIC, PBSpecies::MUDKIP] => PBSpecies::TORIPCKO,
	[PBSpecies::TURTWIG, PBSpecies::CHIMCHAR, PBSpecies::PIPLUP] => PBSpecies::TURCHARPLUP, 
	[PBSpecies::SNIVY, PBSpecies::TEPIG, PBSpecies::OSHAWOTT] => PBSpecies::TESHAVY,
	[PBSpecies::VAPOREON, PBSpecies::JOLTEON, PBSpecies::FLAREON] => PBSpecies::VAPFLOLTEON,
	[PBSpecies::RATTATA, PBSpecies::EKANS, PBSpecies::SANDSHREW] => PBSpecies::EKSHRATA
  ...
  #=============================================================================
  chosen_indices = []
  first_index = $Trainer.party.index(pokemon)
  chosen_indices.push(first_index)
  
  if $Trainer.party.length < REQUIRED_POKEMON_COUNT
    scene.pbDisplay(_INTL("파티에 합체에 필요한 {1}마리의 포켓몬이 없습니다.", REQUIRED_POKEMON_COUNT))
    next false
  end

  # --- 두 번째 포켓몬 선택 ---
  chosen2 = scene.pbChoosePokemon(_INTL("두 번째 합체 포켓몬을 선택하세요."))
  if chosen2 < 0 || chosen2 == first_index
    if chosen2 == first_index
      scene.pbDisplay(_INTL("다른 포켓몬을 선택해야 합니다."))
    end
    next false
  end
  chosen_indices.push(chosen2)
  
  # --- 세 번째 포켓몬 선택 ---
  chosen3 = scene.pbChoosePokemon(_INTL("세 번째 합체 포켓몬을 선택하세요."))
  
  if chosen3 < 0 || chosen3 == first_index || chosen3 == chosen2
    if chosen3 == first_index || chosen3 == chosen2
      scene.pbDisplay(_INTL("다른 포켓몬을 선택해야 합니다."))
    end
    next false
  end
  chosen_indices.push(chosen3)
  
  #=============================================================================
  # 2. 합체 결과 종 ID 결정 및 레시피 확인 (추가 기능 2)
  #=============================================================================
  
  # 1. 선택된 3마리 포켓몬의 종 ID를 가져와서 정렬합니다.
  species_list = []
  chosen_indices.each do |i|
    species_list.push($Trainer.party[i].species)
  end
  species_list.sort! # 어떤 순서로 선택하든 결과가 같도록 오름차순 정렬

  # 2. 레시피 딕셔너리에서 결과 ID를 찾습니다.
  fused_species_id = FUSION_RECIPES[species_list]
  
  if !fused_species_id
    scene.pbDisplay(_INTL("선택한 포켓몬 조합은 합체 레시피에 없습니다."))
    next false
  end

# ... (생략: FUSION_RECIPES 정의 및 포켓몬 선택 로직) ...
  ...
  # 1. 합체 직전 메시지 표시
  scene.pbDisplay(_INTL("DNA 퓨저가 작동하기 시작했습니다..."))

  # 인덱스 정렬 및 베이스 포켓몬 지정
  chosen_indices.sort!
  base_pokemon_index = chosen_indices[0] 
  fused_species_id = FUSION_RECIPES[species_list] # 이미 결정된 합체 종 ID

  # 새로운 합체 포켓몬 객체 생성
  new_pokemon = PokeBattle_Pokemon.new(fused_species_id, $Trainer.party[base_pokemon_index].level)
  
  # --- **화면 전환 및 데이터 갱신 시작** ---
  # Kernel.pbFadeOutInWithMusic 대신 Kernel.pbFadeOutIn을 사용하여 화면 암전 시작
 # Kernel.pbFadeOutIn(99999){ 
    # 2. 파티 내 포켓몬 교체
    $Trainer.party[base_pokemon_index] = new_pokemon

    # 3. 나머지 2마리 포켓몬 제거
    $Trainer.party.delete_at(chosen_indices[2]) 
    $Trainer.party.delete_at(chosen_indices[1])

    # 4. 파티 재정렬
    $Trainer.party.compact!

    # 5. 하드 리프레시를 사용하여 파티 화면을 완전히 다시 그림
    scene.pbHardRefresh 
    
    # 6. 완료 메시지 (화면이 다시 밝아진 후 메시지 표시)
    scene.pbDisplay(_INTL("세 포켓몬이 합쳐져 {1}이(가) 되었습니다!", new_pokemon.name))
 # } 
  # --- **화면 전환 및 데이터 갱신 끝** ---
