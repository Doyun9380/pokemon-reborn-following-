...
class PokeBattle_AI
	...
	def getMoveScore(initialscores=[],scoreindex=-1)
		...
		if @move.basedamage>0
			...
			wondercheck = typemod<=4 && (@opponent.ability == PBAbilities::WONDERGUARD || @opponent.ability == PBAbilities::RENEGADEPLATINUM || @opponent.ability == PBAbilities::NIGHOMNIPOTENCE) 
		end
		...
		if @mondata.skill>=MEDIUMSKILL && !moldBreakerCheck(@attacker) &&
			...
			$ai_log_data[@attacker.index].final_score_moves.push(0) if (@opponent.ability == PBAbilities::MAGICBOUNCE || @opponent.pbPartner.ability == PBAbilities::MAGICBOUNCE || @opponent.ability == PBAbilities::ROYALPINK || @opponent.pbPartner.ability == PBAbilities::ROYALPINK) && @move.basedamage == 0 #there is not a good way to do this section
			...
			return -1 if (@opponent.ability == PBAbilities::MAGICBOUNCE || @opponent.pbPartner.ability == PBAbilities::MAGICBOUNCE || @opponent.ability == PBAbilities::ROYALPINK || @opponent.pbPartner.ability == PBAbilities::ROYALPINK) && @move.basedamage == 0 #there is not a good way to do this section
		...
		#자시아젠투스 전투 전용
		if @opponent.species == PBSpecies::ZACIAZENTUS # 예: 상대가 피카츄일 경우
			if @move.id == PBMoves::BEHEMOTHBLADESTELLAR || @move.id == PBMoves::BEHEMOTHBASHSTELLAR      # 예: 지진을 가지고 있다면
				score += 200 # 점수를 대폭 높여서 최우선순위로 설정
			end
		end
		if @opponent.species == PBSpecies::ZACIAN || @opponent.species == PBSpecies::ZAMAZENTA # 예: 상대가 피카츄일 경우
			if @move.id == PBMoves::ETERNABEAMSTELLAR     # 예: 지진을 가지고 있다면
				score += 200 # 점수를 대폭 높여서 최우선순위로 설정
			end
		end
		...
		case @move.function
			...
			when 0x3b # Superpower
				...
				if @attacker.ability == PBAbilities::CONTRARY || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
					miniscore = selfstatboost(statarray)
				else
			...
			when 0x3e # Hammer Arm, Ice Hammer
				...
				if @attacker.ability == PBAbilities::CONTRARY || @battle.trickroom > 2 || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
					miniscore = selfstatboost(statarray)
				else
					miniscore = selfstatdrop(statarray,score)
				end
				when 0x254 # Spin Out
					statarray = [0,0,2,0,0,0,0]
					if @attacker.ability == PBAbilities::CONTRARY || @battle.trickroom > 2 || @attacker.ability == PBAbilities::UNOVAPOWER
					miniscore = selfstatboost(statarray)
				else
					miniscore = selfstatdrop(statarray,score)
				end
			...
			when 0x100 # Rain Dance
				...
				if @battle.pbWeather==PBWeather::SUNNYDAY #Making Rainbow Field
					miniscore*= getFieldDisruptScore(@attacker,@opponent)
					miniscore*=1.2 if @attacker.pbHasType?(:NORMAL)
					miniscore=9999 if (@battle.opponent.is_a?(Array) ? @battle.opponent[0].trainertype : @battle.opponent.trainertype)==PBTrainers::GALACTICWANDERER
				end
				if !@battle.opponent.is_a?(Array)
			...
			when 0x105, 0x220 # Stealth Rocks / Stone Axe
				...
				if @mondata.skill>=BESTSKILL
					if @battle.FE == PBFields::WASTELAND && !(@opponent.ability==PBAbilities::MAGICBOUNCE || @opponent.pbPartner.ability==PBAbilities::MAGICBOUNCE || @opponent.ability == PBAbilities::ROYALPINK || @opponent.pbPartner.ability == PBAbilities::ROYALPINK) &&
			...
			when 0x279 # Revival Blessing
				...
				if @attacker.pbNonActivePokemonCount <= 2
					miniscore *= 10
				end

				# 체력이 낮을수록 Revival Blessing을 더 우선
...
#All functions here return a modifier to the original score, similar to miniscore
	...
	def selfstatboost(stats)
		...
		if move.basedamage>0
			...
			miniscore*=0 if @attacker.ability == PBAbilities::CONTRARY || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
	...
	def oppstatboost(stats)
		...
		if @opponent.index != @attacker.pbPartner.index
			...
			miniscore*=0 if @opponent.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
	...
	def oppstatdrop(stats)
		...
		return @move.basedamage > 0 ? 1 : 0 if @opponent.ability == PBAbilities::CLEARBODY || @opponent.ability == PBAbilities::WHITESMOKE || @opponent.ability == PBAbilities::GOODASGOLD || @opponent.ability == PBAbilities::SOULHEART
		#stats should be an array of the stat boosts like so: [ATK,DEF,SPE,SPA,SPD,ACC,EVA] with nils in unaffected stats
		#coil, for example, would be [1,1,0,0,0,1,0]
		stats.unshift(0) #this is required to make the next line work correctly
		...
		if stats[PBStats::SPEED]>0  #speed stuff
			...
			miniscore*=0.2 if @opponent.ability == PBAbilities::COMPETITIVE || @opponent.ability == PBAbilities::DEFIANT || @opponent.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
		else    #non-speed stuff
			miniscore*=0.1 if @opponent.ability == PBAbilities::COMPETITIVE || @opponent.ability == PBAbilities::DEFIANT || @opponent.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
			miniscore*=1.1 if @mondata.partyroles.any? {|roles| roles.include?(PBMonRoles::SWEEPER)}
		end
	...
	def typechangecode(type)
		...
		return 0 if @attacker.ability == PBAbilities::MULTITYPE || @attacker.ability == PBAbilities::RKSSYSTEM || @attacker.ability == PBAbilities::PROTEAN || @attacker.ability == PBAbilities::COLORCHANGE || @attacker.ability == PBAbilities::ORIGINOFTHESPECIES
	...
	def opptypechangecode(type)
		...
		return 0 if @opponent.ability == PBAbilities::MULTITYPE || @opponent.ability == PBAbilities::RKSSYSTEM || @opponent.ability == PBAbilities::PROTEAN || @opponent.ability == PBAbilities::COLORCHANGE || @opponent.ability == PBAbilities::ORIGINOFTHESPECIES
	...
	def mooncode
		return 0 if (@attacker.effects[PBEffects::BloodMoon] && @attacker.lastMoveUsed == PBMoves::BLOODMOON) || (@attacker.effects[PBEffects::BloodMoon] && @attacker.lastMoveUsed == PBMoves::KAMEHAMEHA)
		miniscore = 1.0
		return miniscore
	end
...
miniscore*=5   if (@battle.FE == PBFields::GRASSYT && @attacker.species == PBSpecies::ROTOM) && @battle.pbWeather != PBWeather::RAINDANCE # Madame Meganium battle
	...
	def flowershieldcode(score)
		...
		if (@battle.FE == PBFields::FLOWERGARDENF && @battle.field.counter>0) || @battle.FE == PBFields::FAIRYTALEF
			...
			score=0 if @attacker.ability == PBAbilities::CONTRARY || @opponent.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
		end
		return score
	end
	...
	def noLeechSeed(leechTarget)
		...
		return true if (leechTarget.ability == PBAbilities::MAGICBOUNCE || leechTarget.ability == PBAbilities::ROYALPINK)
	...
	def spectralthiefcode
		...
		miniscore*=(-1) if @attacker.ability == PBAbilities::CONTRARY || @attacker.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
...
=end
	...
	def pbAIfaster?(attackermove=nil, opponentmove=nil, attacker=@attacker, opponent=@opponent)
		...
		for battler in [attacker, opponent]
			...
			pri += 1 if @battle.FE == PBFields::FORESTF && battlermove.id == PBMoves::JUNGLEGLIDE
			...
			pri += 1 if (battler.ability == PBAbilities::BRILLIANTDIAMOND) || (battler.ability == PBAbilities::NIGHOMNIPOTENCE)
			...
			pri += 3 if battler.ability == PBAbilities::INTREPIDSWORD && battlermove.type==PBTypes::STELLAR
			pri += 3 if battler.ability == PBAbilities::DAUNTLESSSHIELD && battlermove.type==PBTypes::STELLAR
		...
		case opponent.ability
			...
			when PBAbilities::PLOTARMOR
				...
				if attacker.moves.any? {|moveloop| moveloop!=nil && moveloop.pbType(attacker) == (PBTypes::NUCLEAR || PBTypes::COSMIC)}
					...
					abilityscore*=2 if cosmicmove && pbTypeModNoMessages(cosmicmove.pbType(attacker),attacker,opponent,cosmicmove)>4
				end
			when PBAbilities::LEVITATE,PBAbilities::EARTHEATER
				for i in attacker.moves
					next if i.nil?
			...
			when PBAbilities::SHEERFORCE
				abilityscore*=1.2
			when PBAbilities::CONTRARY || PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
				abilityscore*=1.4
...
################################################################################
	...
	def getSwitchingScore
		...
		if @attacker.ability == PBAbilities::ZEROTOHERO
			@mondata.shouldswitchscore = 9999
		end
	...
	def getSwitchInScoresParty(hard_switch)
		...
		for partyindex in 0...party.length
			...
			if @opponent.species == PBSpecies::ZACIAZENTUS # 예: 상대가 피카츄일 때
				if i.species == PBSpecies::ZAMAZENTA     # 내 포켓몬 중 그란돈이 있다면
					monscore += 500 # 점수를 매우 크게 높여 최우선순위로 설정
				end
			end
	...
	def shouldSwitch?
		...
		betterswitchscore+=250 if @attacker.ability == PBAbilities::ZEROTOHERO
	...
	def pbStatChangingSwitch(mon)
		...
		if mon.pbOwnSide.effects[PBEffects::StickyWeb] && !mon.isAirborne?
			...
			mon.stages[PBStats::SPEED]-= drop unless mon.item == PBItems::WHITEHERB || mon.ability == PBAbilities::WHITESMOKE || mon.ability == PBAbilities::CLEARBODY || mon.ability == PBAbilities::SOULHEART
		...
		#Contrary
		if mon.ability==PBAbilities::CONTRARY || mon.ability == PBAbilities::ALOLANTECHNIQUESB || @attacker.ability == PBAbilities::UNOVAPOWER
...
######################################################
	def pbRoughDamage(move=@move,attacker=@attacker,opponent=@opponent)
		...
		if @mondata.skill>=MEDIUMSKILL
		  ############ ATTACKER ABILITY CHECKS ############
			...
			# Iron Fist
			elsif (attacker.ability == PBAbilities::IRONFIST || attacker.item == PBItems::PUNCHINGGLOVE || attacker.ability == PBAbilities::LIGHTNINGPUNCHER)
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			if opponent.ability == PBAbilities::SOLIDROCK || opponent.ability == PBAbilities::FILTER || opponent.ability == PBAbilities::PRISMARMOR || opponent.ability == PBAbilities::NEUROFORCE
	...
	def moveSuccesful?(move,attacker,opponent)
		if move.pbIsPriorityMoveAI(attacker)
			...
			return false if @battle.FE == PBFields::FORESTF && move.id == PBMoves::JUNGLEGLIDE
		end
		return true
	end
