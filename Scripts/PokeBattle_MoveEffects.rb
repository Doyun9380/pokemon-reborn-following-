...
class PokeBattle_Move_000 < PokeBattle_Move
  ...
  if @id == PBMoves::LANDSWRATH || @id == PBMoves::LANDSWRATH || @id == PBMoves::LANDSWRATH 
    ...
    if attacker.class == PokeBattle_Battler
      ...
      atkmult *= 1.5 if (attacker.ability == PBAbilities::GUTS || attacker.ability == PBAbilities::MOLECULARFLUX) && attacker.status!=0
      ...
      atkmult *= 0.5 if attacker.status==PBStatuses::BURN && !((attacker.ability == PBAbilities::GUTS || attacker.ability == PBAbilities::MOLECULARFLUX || attacker.ability == PBAbilities::SINISTERBLAZE) && attacker.status!=0)
    end
    ...
    if storedatk>storedspatk
      return true
    else #attacker.species == PBSpecies::ZYGARDE && attacker.ability == PBAbilities::AURABREAK
      return false
    end
  ...
  def pbIsSpecial?(type=@type)
  #if (attacker.species == PBSpecies::ZYGARDE) && (attacker.ability == PBAbilities::AURABREAK)
...
class PokeBattle_Move_03A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.hasWorkingAbility(:CONTRARY) || attacker.hasWorkingAbility(:ALOLANTECHNIQUESB) || attacker.hasWorkingAbility(:UNOVAPOWER)
...
class PokeBattle_Move_045 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
	...
	if attacker.species == PBSpecies::LUNALA && @id == PBMoves::MOONBLAST
	ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
      hpgain=((opponent.damagestate.hplost+1)*0.50).floor
...
class PokeBattle_Move_04A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    opponent.ability == PBAbilities::WHITESMOKE || opponent.ability == PBAbilities::FULLMETALBODY || opponent.ability == PBAbilities::SOULHEART || opponent.ability == PBAbilities::ROYALPINK ) && !(opponent.moldbroken)
...
class PokeBattle_Move_08B < PokeBattle_Move
  def pbOnStartUse(attacker)
    if @battle.FE == PBFields::CORROSIVEMISTF && @id == PBMoves::ERUPTION
      bearer=@battle.pbCheckGlobalAbility(:DAMP)
	  bearer=@battle.pbCheckGlobalAbility(:HOENNPOWER)
      if bearer
 ...
 # if (@id == PBMoves::TECHNOBLAST)
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @id == PBMoves::JUDGMENT
      ...
      when PBTypes::QMARKS
        @battle.pbAnimation(PBMoves::JUDGMENTQMARKS,attacker,opponent,hitnum)
	  else
        @battle.pbAnimation(id,attacker,opponent,hitnum)
      end
    end
...
# (SacredSword / Chip Away / Darkest Lariat) + Nihil Light 니힐레이저
################################################################################
class PokeBattle_Move_0A9 < PokeBattle_Move
# Handled in superclass, do not edit!
	if @id = PBMoves::NIHILLIGHT
	def pbIsPhysical?(type=@type)
    attacker = @user
    stagemul=[2,2,2,2,2,2,2,3,4,5,6,7,8]
    stagediv=[8,7,6,5,4,3,2,2,2,2,2,2,2]
...
class PokeBattle_Move_0C0 < PokeBattle_Move
  ...
  def pbNumHits(attacker)
    ...
    if @id == PBMoves::WATERSHURIKEN && attacker.species == PBSpecies::GRENINJA && !attacker.effects[PBEffects::Transform]
	  ...
	  ret=3 if attacker.form==3
    end
    return ret
  end
#  if @id = PBMoves::WATERSHURIKEN
  def pbShowAnimation(id,attacker,opponent,hitnum=7,alltargets=nil,showanimation=true)
    return if !showanimation
    if @id == PBMoves::WATERSHURIKEN && attacker.form==2 && attacker.species==PBSpecies::GRENINJA && attacker.pokemon.isShiny?
      @battle.pbAnimation(PBMoves::WATERSHURIKENRED,attacker,opponent,hitnum) #green
	elsif @id == PBMoves::WATERSHURIKEN && attacker.form==2 && attacker.species==PBSpecies::GRENINJA
	  @battle.pbAnimation(PBMoves::WATERSHURIKENGREEN,attacker,opponent,hitnum) #red
    else
      @battle.pbAnimation(id,attacker,opponent,hitnum) #blue
    end
  end
#  end
end
...
class PokeBattle_Move_0C2 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
		if (attacker.ability != PBAbilities::TIMETRAVEL && attacker.ability != PBAbilities::HYPERMOMENTUM && attacker.ability != PBAbilities::BRILLIANTDIAMOND && attacker.ability != PBAbilities::NIGHOMNIPOTENCE && !((@id == PBMoves::BLASTBURN || @id == PBMoves::FRENZYPLANT || @id == PBMoves::HYDROCANNON) && (attacker.ability == PBAbilities::KANTOPOWER || attacker.ability == PBAbilities::JOHTOPOWER || attacker.ability == PBAbilities::HOENNPOWER || attacker.ability == PBAbilities::SINNOHPOWER || attacker.ability == PBAbilities::UNOVAPOWER)))
  ...
  def pbAdditionalEffect(attacker,opponent)
    if opponent.isFainted? && @id == PBMoves::RADIANTASSAULT
      attacker.effects[PBEffects::HyperBeam]=0
    end
  end
end
...
class PokeBattle_Move_0C4 < PokeBattle_Move
  def pbTwoTurnAttack(attacker,checking=false)
    ...
    if attacker.effects[PBEffects::TwoTurnAttack]==0
	  ...
	  @immediate=true if attacker.ability == PBAbilities::TIMETRAVEL
	  @immediate=true if attacker.ability == PBAbilities::BRILLIANTDIAMOND
	  @immediate=true if attacker.ability == PBAbilities::NIGHOMNIPOTENCE
...
class PokeBattle_Move_0CD < PokeBattle_Move
  def pbTwoTurnAttack(attacker,checking=false)
	...
	elsif !@immediate && (attacker.hasWorkingAbility(:RENEGADEPLATINUM) || attacker.hasWorkingAbility(:NIGHOMNIPOTENCE))
		@immediate=true
    end
    return false if @immediate
    return attacker.effects[PBEffects::TwoTurnAttack]==0
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !opponent.isFainted? && opponent.damagestate.calcdamage>0 &&
      ...
      if opponent.effects[PBEffects::MultiTurn]==0
		...
		elsif (@id == PBMoves::STABBYSWARM)
          @battle.pbDisplay(_INTL("{1} was trapped in a stinging swarm!",opponent.pbThis,attacker.pbThis(true)))
        else
          @battle.pbDisplay(_INTL("{1} was trapped in the vortex!",opponent.pbThis))
        end
      end
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.isFainted? && @battle.pbCanChooseNonActive?(attacker.index) &&
      ...
      if @id == PBMoves::VOLTSWITCH && (opponent.ability == PBAbilities::MOTORDRIVE ||
		...
		opponent.ability == PBAbilities::LIGHTNINGPUNCHER ||
        opponent.ability == PBAbilities::LIGHTNINGROD ||
		opponent.ability == PBAbilities::HOENNPOWER)
        attacker.userSwitch = false
        attacker.vanished=false
      end
...
class PokeBattle_Move_0FA < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.damagestate.calcdamage>0 &&
	   ...
	   attacker.ability != PBAbilities::HYPERMOMENTUM &&
...
class PokeBattle_Move_0FF < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    elsif @battle.weather==PBWeather::STRONGWINDS && (@battle.pbCheckGlobalAbility(:CATACLYSM)) 
      @battle.pbDisplay(_INTL("The mysterious air current blows on regardless!"))
      return -1
    end
...
class PokeBattle_Move_103 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    for i in 0...4
      ...
      if ((@battle.battlers[i].ability == PBAbilities::MAGICBOUNCE || @battle.battlers[i].ability == PBAbilities::ROYALPINK || @battle.battlers[i].ability == PBAbilities::MAGICCHILL) && !PBStuff::TWOTURNMOVE.include?(@battle.battlers[i].effects[PBEffects::TwoTurnAttack])) || 
...
class PokeBattle_Move_111 < PokeBattle_Move
 ...
 # if (@id == PBMoves::DOOMDUMMY)
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    # ★ TIMETRAVEL 즉시 발동
    #---------------------------------------------------------------------------
    if attacker.ability == PBAbilities::TIMETRAVEL || attacker.ability == PBAbilities::BRILLIANTDIAMOND || attacker.ability == PBAbilities::NIGHOMNIPOTENCE 
      @battle.pbDisplay(_INTL("{1} bent time to strike instantly!", attacker.pbThis))

      # 타입 상성 및 무적·특성 판정 준비
      typemod      = self.pbTypeModifier(self.type, attacker, opponent)
      twoturninvul = PBStuff::TWOTURNMOVE.include?(opponent.effects[PBEffects::TwoTurnAttack])

      # 원래 Future Sight와 동일한 명중/면역/특성 조건
      if (opponent.isFainted? ||
          pbAccuracyCheck(attacker, opponent) &&
          typemod > 0 &&                                      # 타입 면역(0배) 차단
          !((opponent.ability == PBAbilities::WONDERGUARD || opponent.ability == PBAbilities::RENEGADEPLATINUM || opponent.ability == PBAbilities::NIGHOMNIPOTENCE ) && typemod <= 4)
         ) && !twoturninvul

        opponent.damagestate.reset

        # 공격 애니메이션 (예언 실현)
        if @id == PBMoves::FUTURESIGHT && !opponent.pbHasType?(:DARK)
          @battle.pbAnimation(PBMoves::FUTUREDUMMY, attacker, opponent)
        elsif @id == PBMoves::DOOMDESIRE
          @battle.pbAnimation(PBMoves::DOOMDUMMY, attacker, opponent)
        end

        # 데미지 계산 및 적용
        damage = pbCalcDamage(attacker, opponent)
        pbReduceHPDamage(damage, attacker, opponent)
        pbEffectMessages(attacker, opponent)

      elsif ((opponent.ability == PBAbilities::WONDERGUARD || opponent.ability == PBAbilities::RENEGADEPLATINUM || opponent.ability == PBAbilities::NIGHOMNIPOTENCE )) && typemod <= 4 && !twoturninvul
        @battle.pbDisplay(_INTL("{1} avoided damage with Wonder Guard!", opponent.pbThis))
      else
        @battle.pbDisplay(_INTL("But it failed!"))
      end

      # 예언 효과 완전 초기화 → 중복 발동 방지
      opponent.effects[PBEffects::FutureSight]              = 0
      opponent.effects[PBEffects::FutureSightMove]          = 0
      opponent.effects[PBEffects::FutureSightUser]          = -1
      opponent.effects[PBEffects::FutureSightPokemonIndex]  = -1
      return 0
    end
...
class PokeBattle_Move_13E < PokeBattle_Move
  def pbTwoTurnAttack(attacker)
	...
	elsif !@immediate && (attacker.ability == PBAbilities::TIMETRAVEL)
	  @immediate=true
      @battle.pbDisplay(_INTL("{1} traveled time forward!",attacker.pbThis))
	elsif !@immediate && (attacker.ability == PBAbilities::BRILLIANTDIAMOND || attacker.ability == PBAbilities::NIGHOMNIPOTENCE)
	  @immediate=true
      @battle.pbDisplay(_INTL("{1} manipulated time forward!",attacker.pbThis))
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
	...
	if attacker.pokemon.species == PBSpecies::XERNEAS
		if !attacker.pbCanIncreaseStatStage?(PBStats::SPATK) && !attacker.pbCanIncreaseStatStage?(PBStats::SPDEF) && !attacker.pbCanIncreaseStatStage?(PBStats::SPEED)
			...
			pbShowAnimation(PBMoves::HEALINGWISH,attacker,nil,hitnum,alltargets,showanimation)
			if !@battle.pbOwnedByPlayer?(attacker.index)
			...
			if pokemon
				...
				@battle.pbDisplay(_INTL("Xerneas revived {1}!!",pokemon.name))
...
class PokeBattle_Move_155 < PokeBattle_Move
 if (@id == PBMoves::THOUSANDWAVES)
  def pbIsPhysical?(type=@type)
    ...
    atkstage=6
    atkmult = 1.0
	#if (attacker.species == PBSpecies::ZYGARDE) && (attacker.ability == PBAbilities::AURABREAK) && (@id == PBMoves::THOUSANDWAVES)
    if attacker.class == PokeBattle_Battler
      atkstage=attacker.stages[PBStats::ATTACK]+6
      atkmult *= 1.5 if attacker.hasWorkingItem(:CHOICEBAND)
      atkmult *= 1.5 if attacker.ability == PBAbilities::HUSTLE
...
class PokeBattle_Move_178 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
	...
	if @id==PBMoves::ETERNABEAMSTELLAR && (opponent.species == PBSpecies::ZACIAN || opponent.species == PBSpecies::ZAMAZENTA)
	  basedmg*=10
	end
    return basedmg
  end  
...
class PokeBattle_Move_201 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    unless @battle.canChangeFE?
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    @battle.breakField   # 이 줄이 핵심!
...
class PokeBattle_Move_203 < PokeBattle_Move
  ...
  if attacker.effects[PBEffects::TwoTurnAttack]==0
    @immediate=true if (attacker.ability == PBAbilities::TIMETRAVEL || attacker.ability == PBAbilities::BRILLIANTDIAMOND || attacker.ability == PBAbilities::NIGHOMNIPOTENCE) 
  end
...
class PokeBattle_Move_212 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
   ...
   if (@id == PBMoves::JUNGLEHEALING)
    ...
    @battle.field.duration=5
    @battle.field.duration=8 if (attacker.item == PBItems::AMPLIFIELDROCK)
    @battle.pbDisplay(_INTL("The field became Forest!"))
	end
	if attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false)
      attacker.pbIncreaseStat(PBStats::ATTACK,2,abilitymessage:false)
    end
...
class PokeBattle_Move_280 < PokeBattle_Move
  def pbType(attacker,type=@type)
    if attacker.species == PBSpecies::OGERPON
	  type = PBTypes::GRASS if attacker.form == 1
	  ...
	  type = PBTypes::COSMIC if attacker.form == 9 || attacker.form == 10
    end
    return super(attacker,type)
  end
end
...
# 이 아래부터 레쿠쟈 버전 추가 기술 효과 코드

class PokeBattle_Move_920 < PokeBattle_Move
    def pbAdditionalEffect(attacker, opponent)
      lowerdef = opponent.pbCanReduceStatStage?(PBStats::DEFENSE, false)
      lowerspeed = opponent.pbCanReduceStatStage?(PBStats::SPEED, false)
       if lowerdef
          opponent.pbReduceStat(PBStats::DEFENSE, 1, abilitymessage: false, statdropper: attacker)
       end
       if lowerspeed
    ...
    def pbShowAnimation(id, attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
      ...
      if id == :PUNCHYPUMMEL
        @battle.pbAnimation(:CLOSECOMBAT, attacker, opponent, hitnum)
      else
        @battle.pbAnimation(id, attacker, opponent, hitnum)
      end
    end
...
class PokeBattle_Move_921 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    # --- 필드 정리 부분 (Reborn 호환 인덱싱) ---
    if attacker.pbOwnSide.effects[PBEffects::StealthRock] ||
       attacker.pbOpposingSide.effects[PBEffects::StealthRock]
      attacker.pbOwnSide.effects[PBEffects::StealthRock] = false
      attacker.pbOpposingSide.effects[PBEffects::StealthRock] = false
      @battle.pbDisplay(_INTL("{1} blew away all stealth rocks!", attacker.pbThis))
      tidy = true
    ...
    if attacker.pbOwnSide.effects[PBEffects::StickyWeb] ||
       attacker.pbOpposingSide.effects[PBEffects::StickyWeb]
      attacker.pbOwnSide.effects[PBEffects::StickyWeb] = false
      attacker.pbOpposingSide.effects[PBEffects::StickyWeb] = false
      @battle.pbDisplay(_INTL("{1} blew away all of the sticky webbing!", attacker.pbThis))
      tidy = true
    ...
    for i in 0...4
      ...
      if battler.effects[PBEffects::Substitute] > 0
        ...
        if @battle.scene.respond_to?(:pbUnSubstituteSprite)
          @battle.scene.pbUnSubstituteSprite(battler, battler.pbIsOpposing?(1))
        end
        subs = true
        tidy = true
      end
...
################################################################################
# Deals damage and starts sandstorm weather. (Custom Dusty Drift)
################################################################################
class PokeBattle_Move_922 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    # 3. 날씨 설정 (공격기가 아니었을 때만 애니메이션 표시)
    pbShowAnimation(@id, attacker, opponent, hitnum, alltargets, showanimation) unless played_animation

    @battle.weather = PBWeather::SANDSTORM
    @battle.weatherduration = 5
    if attacker.item == PBItems::SMOOTHROCK ||
       @battle.FE == PBFields::DESERTF ||
       @battle.FE == PBFields::ASHENB
  ...
  def pbAdditionalEffect(attacker, opponent)
    unless @battle.weather == PBWeather::SANDSTORM
      @battle.weather = PBWeather::SANDSTORM
      @battle.weatherduration = 5
      if attacker.item == PBItems::SMOOTHROCK ||
         @battle.FE == PBFields::DESERTF ||
  ...
  def pbShowAnimation(id, attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    return if !showanimation
    if id == :DUSTYDRIFT
      @battle.pbAnimation(:SANDSTORM, attacker, opponent, hitnum)
    else
      @battle.pbAnimation(id, attacker, opponent, hitnum)
    end
  end
...
################################################################################
# Judgment Stellar (심판의 뭉치 스텔라)
################################################################################
class PokeBattle_Move_923 < PokeBattle_Move
end

################################################################################
# 거수참 스텔라 BEHEMOTHBLADESTELLAR BEHEMOTH BLADE STELLAR
################################################################################
class PokeBattle_Move_924 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if opponent.isMega? || opponent.isUltra? #|| opponent.isGiga?
      basedmg*=2
    end
  ...
  #if @id == PBMoves::BEHEMOTHBLADESTELLAR
    def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
	...
	opponent.pbReduceStat(PBStats::SPEED,5,abilitymessage:false, statdropper: attacker)
	opponent.pbReduceStat(PBStats::DEFENSE,6,abilitymessage:false, statdropper: attacker)
	opponent.pbReduceStat(PBStats::SPDEF,6,abilitymessage:false, statdropper: attacker)
	opponent.pbReduceStat(PBStats::EVASION,4,abilitymessage:false, statdropper: attacker)
    @battle.pbDisplay(_INTL("자시안이 상대방의 방어막을 없앴다!!")) if subs
...
################################################################################
# 거수탄 스텔라 BEHEMOTHBASHSTELLAR BEHEMOTH BASH STELLAR
################################################################################
class PokeBattle_Move_926 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if opponent.isMega? || opponent.isUltra? #|| opponent.isGiga?
      basedmg*=2
    end
	...
	opponent.hp = (opponent.hp*0.50).floor
	opponent.pbReduceStat(PBStats::ACCURACY,5,abilitymessage:false, statdropper: attacker)
	opponent.pbReduceStat(PBStats::ATTACK,6,abilitymessage:false, statdropper: attacker)
	opponent.pbReduceStat(PBStats::SPATK,6,abilitymessage:false, statdropper: attacker)
    @battle.pbDisplay(_INTL("자마젠타가 상대방에게 치명타를 날렸다!!"))
...
# Terrain Kick
################################################################################
class PokeBattle_Move_927 < PokeBattle_Move
  def pbIsPhysical?(type=@type)
  ...
  def pbIsSpecial?(type=@type)
    ...
    storedatk *= ((stagemul[atkstage].to_f / stagediv[atkstage]) * atkmult)

    # --- 특수 공격력 계산 (Special Stuff) ---
    storedspatk = attacker.spatk
    spatkstage = 6
    spatkmult = 1.0
    if attacker.class == PokeBattle_Battler
    ...
    storedspatk *= ((stagemul[spatkstage].to_f / stagediv[spatkstage]) * spatkmult)
    storedspatk = attacker.getSpecialStat if @battle.FE == PBFields::GLITCHF && attacker.class == PokeBattle_Battler

    # --- 최종 판정 (특수가 더 높거나 같으면 특수기로 판정) ---
    if storedspatk >= storedatk
      return true # 특수 기술로 취급
    else
      return false # 물리 기술로 취급
    end
  end
  
  def pbType(attacker,type=@type)
      if @id == PBMoves::TERRAINKICK
        case @battle.FE
		  when PBFields::ELECTRICT then type = PBTypes::ELECTRIC
		  when PBFields::GRASSYT then type = PBTypes::GRASS
		  when PBFields::MISTYT then type = PBTypes::FAIRY
		  when PBFields::DARKCRYSTALC then type = PBTypes::DARK
		  when PBFields::CHESSB then type = PBTypes::PSYCHIC
		  when PBFields::BIGTOPA then type = PBTypes::FIGHTING
		  when PBFields::BURNINGF then type = PBTypes::FIRE
		  when PBFields::SWAMPF then type = PBTypes::WATER
		  when PBFields::RAINBOWF then type = PBTypes::DRAGON
		  when PBFields::CORROSIVEF then type = PBTypes::POISON
		  when PBFields::CORROSIVEMISTF then type = PBTypes::POISON
		  when PBFields::DESERTF then type = PBTypes::GROUND
		  when PBFields::ICYF then type = PBTypes::ICE
		  when PBFields::ROCKYF then type = PBTypes::ROCK
		  when PBFields::FORESTF then type = PBTypes::BUG
		  when PBFields::SUPERHEATEDF then type = PBTypes::FIRE
		  when PBFields::FACTORYF then type = PBTypes::STEEL
		  when PBFields::SHORTCIRCUITF then type = PBTypes::GHOST
		  when PBFields::WASTELAND then type = PBTypes::POISON
		  when PBFields::ASHENB then type = PBTypes::GROUND
		  when PBFields::WATERS then type = PBTypes::WATER
		  when PBFields::UNDERWATER then type = PBTypes::WATER
		  when PBFields::CAVE then type = PBTypes::ROCK
		  when PBFields::GLITCHF then type = PBTypes::QMARKS
		  when PBFields::CRYSTALC then type = PBTypes::ROCK
		  when PBFields::MURKWATERS then type = PBTypes::POISON
		  when PBFields::MOUNTAIN then type = PBTypes::FLYING
		  when PBFields::SNOWYM then type = PBTypes::ICE
		  when PBFields::HOLYF then type = PBTypes::NORMAL
		  when PBFields::MIRRORA then type = PBTypes::STEEL
		  when PBFields::FAIRYTALEF then type = PBTypes::FAIRY
		  when PBFields::DRAGONSD then type = PBTypes::DRAGON
		  when PBFields::FLOWERGARDENF then type = PBTypes::GRASS
		  when PBFields::STARLIGHTA then type = PBTypes::DARK
		  when PBFields::NEWW then type = PBTypes::QMARKS
		  when PBFields::INVERSEF then type = PBTypes::NORMAL
		  when PBFields::PSYCHICT then type = PBTypes::PSYCHIC
		  when PBFields::GUFIELD then type = PBTypes::COSMIC
		  when PBFields::DWORLD then type = PBTypes::COSMIC
		  when PBFields::FALLOUT then type = PBTypes::NUCLEAR 
		  when PBFields::NONE then type = PBTypes::NORMAL 
        end
      end
    return super(attacker,type)
  end
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @id == PBMoves::TERRAINKICK
      case movetype
      when PBTypes::FIGHTING
        @battle.pbAnimation(PBMoves::TERRAINKICKFIGHTING,attacker,opponent,hitnum)
      when PBTypes::FLYING
        @battle.pbAnimation(PBMoves::TERRAINKICKFLYING,attacker,opponent,hitnum)
      when PBTypes::POISON
        @battle.pbAnimation(PBMoves::TERRAINKICKPOISON,attacker,opponent,hitnum)
      when PBTypes::GROUND
        @battle.pbAnimation(PBMoves::TERRAINKICKGROUND,attacker,opponent,hitnum)
      when PBTypes::ROCK
        @battle.pbAnimation(PBMoves::TERRAINKICKROCK,attacker,opponent,hitnum)
      when PBTypes::BUG
        @battle.pbAnimation(PBMoves::TERRAINKICKBUG,attacker,opponent,hitnum)
      when PBTypes::GHOST, PBTypes::COSMIC
        @battle.pbAnimation(PBMoves::TERRAINKICKGHOST,attacker,opponent,hitnum)
      when PBTypes::STEEL
        @battle.pbAnimation(PBMoves::TERRAINKICK,attacker,opponent,hitnum)
      when PBTypes::FIRE
        @battle.pbAnimation(PBMoves::TERRAINKICKFIRE,attacker,opponent,hitnum)
      when PBTypes::WATER
        @battle.pbAnimation(PBMoves::TERRAINKICKWATER,attacker,opponent,hitnum)
      when PBTypes::GRASS, PBTypes::NUCLEAR
        @battle.pbAnimation(PBMoves::TERRAINKICKGRASS,attacker,opponent,hitnum)
      when PBTypes::ELECTRIC
        @battle.pbAnimation(PBMoves::TERRAINKICKELECTRIC,attacker,opponent,hitnum)
      when PBTypes::PSYCHIC
        @battle.pbAnimation(PBMoves::TERRAINKICKPSYCHIC,attacker,opponent,hitnum)
      when PBTypes::ICE
        @battle.pbAnimation(PBMoves::TERRAINKICKICE,attacker,opponent,hitnum)
      when PBTypes::DRAGON
        @battle.pbAnimation(PBMoves::TERRAINKICKDRAGON,attacker,opponent,hitnum)
      when PBTypes::DARK 
        @battle.pbAnimation(PBMoves::TERRAINKICKDARK,attacker,opponent,hitnum)
      when PBTypes::FAIRY
        @battle.pbAnimation(PBMoves::TERRAINKICKFAIRY,attacker,opponent,hitnum)
      when PBTypes::QMARKS
        @battle.pbAnimation(PBMoves::TERRAINKICKQMARKS,attacker,opponent,hitnum)
	  else
        @battle.pbAnimation(id,attacker,opponent,hitnum)
      end
    end
...
class PokeBattle_Move_923 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    for i in 0...4
      ...
      if ((@battle.battlers[i].ability == :MAGICBOUNCE || @battle.battlers[i].ability == :MAGICCHILL) && !PBStuff::TWOTURNMOVE.include?(@battle.battlers[i].effects[:TwoTurnAttack])) ||
  ...
  def pbShowAnimation(id, attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    elsif id == :ROCKYRAMPAGE
      @battle.pbAnimation(:ROCKWRECKER, attacker, opponent, hitnum)
    else
      @battle.pbAnimation(id, attacker, opponent, hitnum)
    end
  end
    ...
    if !opponent.isFainted? && opponent.damagestate.calcdamage > 0 && !opponent.damagestate.substitute
      if opponent.effects[:MultiTurn] == 0
        ...
        case @move
          ...
          when :STABBYSWARM then @battle.pbDisplay(_INTL("{1} was trapped in a stinging swarm!", opponent.pbThis))
          else
            @battle.pbDisplay(_INTL("{1} was trapped in the vortex!", opponent.pbThis))
        end
      end
...
/
################################################################################
# Deals damage and then lays a Curse effect on the target if applicable.
################################################################################
class PokeBattle_Move_925 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    ...
    if !opponent.isFainted? &&
       ...
       !opponent.damagestate.substitute &&
       opponent.ability != :SHIELDDUST &&
       opponent.item != :COVERTCLOAK &&
       attacker.ability != :SHEERFORCE

      # 이미 Curse 상태가 아닌 경우에만
      if !opponent.effects[PBEffects::Curse]
        opponent.effects[PBEffects::Curse] = true
        @battle.pbAnimation(:CURSE, attacker, opponent, hitnum)
        @battle.pbDisplay(_INTL("{1} laid a curse on {2}!", attacker.pbThis, opponent.pbThis(true)))
      end
    end
    return ret
  ...
  def pbShowAnimation(id, attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if id == :STEELYSTRIKE
      @battle.pbAnimation(:STEELWING, attacker, opponent, hitnum)
    else
      @battle.pbAnimation(id, attacker, opponent, hitnum)
    end
  end
...
class PokeBattle_Move_927 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker, opponent)
    ...
    increment = 2 if @battle.FE == :DRAGONSDEN && @move == :SCALYSCORN
