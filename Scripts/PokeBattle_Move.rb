...
# Note: I tested most edge cases of this - although I could've missed something
  ...
  def pbDragonDartTargetting(attacker)
    ...
    for i in [opp1,opp2]
      ...
      next if eff == 0
      next if (i.ability == PBAbilities::WONDERGUARD || i.ability == PBAbilities::RENEGADEPLATINUM || i.ability == PBAbilities::NIGHOMNIPOTENCE) && !i.moldbroken && eff <= 4
...
################################################################################
  def pbTypeModifier(type,attacker,opponent,zorovar=false)
    ...
    for otype in [otype1, otype2]
	  ...
	  if (attacker.species == PBSpecies::LUNALA || attacker.species == PBSpecies::NECROZMA) && (@id == PBMoves::MOONGEISTBEAM || @id == PBMoves::MENACINGMOONRAZEMAELSTROM)
        mod=2 if otype == PBTypes::NORMAL && (atype == PBTypes::GHOST)
      end
	  if (attacker.species == PBSpecies::IRONSTREAM) && (@id == PBMoves::PLASMASABER)
        mod=2 if otype == PBTypes::GROUND && (atype == PBTypes::ELECTRIC)
      end
	  if (attacker.species == PBSpecies::ZYGARDE || attacker.ability == PBAbilities::AURABREAK) && (@id == PBMoves::COREENFORCER || @id == PBMoves::NIHILLIGHT)
        mod=2 if otype == PBTypes::FAIRY && (atype == PBTypes::DRAGON)
      end
	  ...
	  if attacker.ability == PBAbilities::ASCENDANCE
		mod=2 if otype == PBTypes::NORMAL && (atype == PBTypes::GHOST)
		mod=2 if otype == PBTypes::GROUND && (atype == PBTypes::ELECTRIC)
		mod=2 if otype == PBTypes::FLYING && (atype == PBTypes::GROUND)
		mod=2 if otype == PBTypes::GHOST && (atype == PBTypes::NORMAL || atype == PBTypes::FIGHTING)
		mod=2 if otype == PBTypes::DARK && (atype == PBTypes::PSYCHIC)
		mod=2 if otype == PBTypes::STEEL && (atype == PBTypes::POISON)
		mod=2 if otype == PBTypes::FAIRY && (atype == PBTypes::DRAGON)
		mod=2 if otype == PBTypes::COSMIC && (atype == PBTypes::FIRE || atype == PBTypes::NUCLEAR)
	  end
	  if attacker.ability == PBAbilities::ECLIPSE
		mod=2 if otype == PBTypes::DARK && (atype == PBTypes::PSYCHIC)
	  end
      ...
      if attacker.ability == PBAbilities::OMNIPOTENT || attacker.ability == PBAbilities::GODHEAD
        mod=2
      end
      if opponent.ability == PBAbilities::TERASHELL && opponent.hp==opponent.totalhp && !(attacker.ability == PBAbilities::OMNIPOTENT || attacker.ability == PBAbilities::GODHEAD)
        mod=1 if mod!=0
      end
	  if opponent.ability == PBAbilities::GODHEAD && !(attacker.ability == PBAbilities::OMNIPOTENT || attacker.ability == PBAbilities::GODHEAD)
        mod=1 if mod!=0
      end
    ...
    if !opponent.moldbroken
      if (atype == PBTypes::FIRE && (opponent.ability == PBAbilities::FLASHFIRE || opponent.ability == PBAbilities::WELLBAKEDBODY || opponent.ability == PBAbilities::WARMBLANKET || opponent.ability == PBAbilities::HOTBLOODED || opponent.ability == PBAbilities::ABYSSALFURNACE || opponent.ability == PBAbilities::MOLECULARFLUX || opponent.ability == PBAbilities::JOHTOPOWER))  ||
        ...
        (atype == PBTypes::WATER && (opponent.ability == PBAbilities::WATERABSORB || opponent.ability == PBAbilities::STORMDRAIN || opponent.ability == PBAbilities::MOLECULARFLUX || opponent.ability == PBAbilities::DRYSKIN || opponent.ability == PBAbilities::COLDBLOODED)) ||
        (atype == PBTypes::ELECTRIC && (opponent.ability == PBAbilities::VOLTABSORB || opponent.ability == PBAbilities::LIGHTNINGROD || opponent.ability == PBAbilities::HOENNPOWER || opponent.ability == PBAbilities::MOLECULARFLUX || opponent.ability == PBAbilities::MOTORDRIVE || opponent.ability == PBAbilities::LIGHTNINGPUNCHER)) ||
  ...
  def pbTypeModifierNonBattler(type,attacker,opponent)
	...
	if (attacker.species == PBSpecies::LUNALA || attacker.species == PBSpecies::NECROZMA) && (@id == PBMoves::MOONGEISTBEAM || @id == PBMoves::MENACINGMOONRAZEMAELSTROM)
      mod1=2 if (otype1 == PBTypes::NORMAL) && (atype == PBTypes::GHOST)
      mod2=2 if (otype2 == PBTypes::NORMAL) && (atype == PBTypes::GHOST)
    end
	if (attacker.species == PBSpecies::IRONSTREAM) && (@id == PBMoves::PLASMASABER)
      mod1=2 if (otype1 == PBTypes::GROUND) && (atype == PBTypes::ELECTRIC)
      mod2=2 if (otype1 == PBTypes::GROUND) && (atype == PBTypes::ELECTRIC)
    end
  ...
  def pbStatusMoveAbsorption(type,attacker,opponent)
    ...
    if (opponent.ability == PBAbilities::STORMDRAIN && type == PBTypes::WATER) || ((opponent.ability == PBAbilities::LIGHTNINGROD || opponent.ability == PBAbilities::HOENNPOWER) && type == PBTypes::ELECTRIC) && !(opponent.moldbroken)
    ...
    if ((opponent.ability == PBAbilities::FLASHFIRE) || (opponent.ability == PBAbilities::ABYSSALFURNACE) || (opponent.ability == PBAbilities::JOHTOPOWER)) && !(opponent.moldbroken) && type == PBTypes::FIRE
      if ((opponent.ability == PBAbilities::ABYSSALFURNACE) && (opponent.pbCanIncreaseStatStage?(PBStats::SPEED)))
          opponent.pbIncreaseStatBasic(PBStats::SPEED,6)
          @battle.pbCommonAnimation("StatUp",opponent,nil)
          @battle.pbDisplay(_INTL("{1}'s {2} maxed out its Speed!",opponent.pbThis,PBAbilities.getName(opponent.ability)))
	  end
  ...
  def pbTypeModMessages(type,attacker,opponent)
    ...
    if (opponent.ability == PBAbilities::STORMDRAIN && (type == PBTypes::WATER || secondtype==PBTypes::WATER)) ||
       ((opponent.ability == PBAbilities::LIGHTNINGROD || opponent.ability == PBAbilities::HOENNPOWER) && (type == PBTypes::ELECTRIC || secondtype==PBTypes::ELECTRIC)) && !(opponent.moldbroken)
      if opponent.pbCanIncreaseStatStage?(PBStats::SPATK) && opponent.pbCanIncreaseStatStage?(PBStats::ATTACK)
        opponent.pbIncreaseStatBasic(PBStats::ATTACK,1)
        opponent.pbIncreaseStatBasic(PBStats::SPATK,1)
        @battle.pbCommonAnimation("StatUp",opponent,nil)
    ...
    if ((opponent.ability == PBAbilities::DRYSKIN && !(opponent.moldbroken)) && (type == PBTypes::WATER || secondtype==PBTypes::WATER)) ||
	  ...
	  (opponent.ability == PBAbilities::LIGHTNINGPUNCHER && !(opponent.moldbroken) && (type == PBTypes::ELECTRIC || secondtype==PBTypes::ELECTRIC))
      if opponent.effects[PBEffects::HealBlock]==0
        if opponent.pbRecoverHP((opponent.totalhp/4.0).floor,true)>0
          @battle.pbDisplay(_INTL("{1}'s {2} restored its HP!",
              opponent.pbThis,PBAbilities.getName(opponent.ability)))
    ...
    if ((opponent.ability == PBAbilities::FLASHFIRE) || (opponent.ability == PBAbilities::ABYSSALFURNACE) || (opponent.ability == PBAbilities::JOHTOPOWER)) && !(opponent.moldbroken) && (type == PBTypes::FIRE || secondtype==PBTypes::FIRE)
     if ((opponent.ability == PBAbilities::ABYSSALFURNACE) && (opponent.pbCanIncreaseStatStage?(PBStats::SPEED)))
          opponent.pbIncreaseStatBasic(PBStats::SPEED,6)
          @battle.pbCommonAnimation("StatUp",opponent,nil)
          @battle.pbDisplay(_INTL("{1}'s {2} maxed out its Speed!",opponent.pbThis,PBAbilities.getName(opponent.ability)))
	...
	typemod *= 4 if (@id == PBMoves::SUNSTEELSTRIKE || @id == PBMoves::SEARINGSUNRAZESMASH) && opponent.hasType?(PBTypes::STEEL) && !@battle.typesInverted?
	
	typemod *= 2 if (@id == PBMoves::MOONGEISTBEAM || @id == PBMoves::MENACINGMOONRAZEMAELSTROM) && opponent.hasType?(PBTypes::NORMAL) && !@battle.typesInverted?
	
	typemod *= 2 if (@id == PBMoves::PLASMASABER) && opponent.hasType?(PBTypes::GROUND) && !@battle.typesInverted?
	
	typemod *= 2 if (attacker.ability == PBAbilities::ECLIPSE && type == PBTypes::PSYCHIC)
	
	typemod *= 2 if (@id == PBMoves::DRAGONBLADE) && opponent.hasType?(PBTypes::DRAGON) && !@battle.typesInverted?
	...
	typemod *= 2 if (@id == PBMoves::JUDGMENTSTELLAR) && attacker.item == PBItems::LEGENDPLATE
	
	typemod *= 0 if (opponent.ability == PBAbilities::ENVIRONMENTALADAPTATION) && ((type == PBTypes::ELECTRIC && @battle.FE == PBFields::ELECTRICT) || (type == PBTypes::GRASS && @battle.FE == PBFields::GRASSYT) || (type == PBTypes::FAIRY && @battle.FE == PBFields::MISTYT) || (type == PBTypes::DARK && @battle.FE == PBFields::DARKCRYSTALC) || (type == PBTypes::PSYCHIC && @battle.FE == PBFields::CHESSB) || (type == PBTypes::FIGHTING && @battle.FE == PBFields::BIGTOPA) || (type == PBTypes::FIRE && @battle.FE == PBFields::BURNINGF) || (type == PBTypes::WATER && @battle.FE == PBFields::SWAMPF) || (type == PBTypes::DRAGON && @battle.FE == PBFields::RAINBOWF) || (type == PBTypes::POISON && @battle.FE == PBFields::CORROSIVEF) || (type == PBTypes::POISON && @battle.FE == PBFields::CORROSIVEMISTF) || (type == PBTypes::GROUND && @battle.FE == PBFields::DESERTF) || (type == PBTypes::ICE && @battle.FE == PBFields::ICYF) || (type == PBTypes::ROCK && @battle.FE == PBFields::ROCKYF) || (type == PBTypes::BUG && @battle.FE == PBFields::FORESTF) || (type == PBTypes::FIRE && @battle.FE == PBFields::SUPERHEATEDF) || (type == PBTypes::STEEL && @battle.FE == PBFields::FACTORYF) || (type == PBTypes::GHOST && @battle.FE == PBFields::SHORTCIRCUITF) || (type == PBTypes::POISON && @battle.FE == PBFields::WASTELAND) || (type == PBTypes::GROUND && @battle.FE == PBFields::ASHENB) || (type == PBTypes::WATER && @battle.FE == PBFields::WATERS) || (type == PBTypes::WATER && @battle.FE == PBFields::UNDERWATER) || (type == PBTypes::ROCK && @battle.FE == PBFields::CAVE) || (type == PBTypes::QMARKS && @battle.FE == PBFields::GLITCHF) || (type == PBTypes::ROCK && @battle.FE == PBFields::CRYSTALC) || (type == PBTypes::POISON && @battle.FE == PBFields::MURKWATERS) || (type == PBTypes::FLYING && @battle.FE == PBFields::MOUNTAIN) || (type == PBTypes::ICE && @battle.FE == PBFields::SNOWYM) || (type == PBTypes::NORMAL && @battle.FE == PBFields::HOLYF) || (type == PBTypes::STEEL && @battle.FE == PBFields::MIRRORA) || (type == PBTypes::FAIRY && @battle.FE == PBFields::FAIRYTALEF) || (type == PBTypes::DRAGON && @battle.FE == PBFields::DRAGONSD) || (type == PBTypes::GRASS && @battle.FE == PBFields::FLOWERGARDENF) || (type == PBTypes::DARK && @battle.FE == PBFields::STARLIGHTA) || (type == PBTypes::QMARKS && @battle.FE == PBFields::NEWW) || (type == PBTypes::NORMAL && @battle.FE == PBFields::INVERSEF) || (type == PBTypes::PSYCHIC && @battle.FE == PBFields::PSYCHICT) || (type == PBTypes::NORMAL && @battle.FE == PBFields::INDOORA) || (type == PBTypes::NORMAL && @battle.FE == PBFields::INDOORB) || (type == PBTypes::NORMAL && @battle.FE == PBFields::INDOORC) || (type == PBTypes::NORMAL && @battle.FE == PBFields::CITY) || (type == PBTypes::NORMAL && @battle.FE == PBFields::CITYNEW) || (type == PBTypes::COSMIC && @battle.FE == PBFields::GUFIELD) || (type == PBTypes::COSMIC && @battle.FE == PBFields::DWORLD) || (type == PBTypes::NUCLEAR && @battle.FE == PBFields::FALLOUT) || (type == PBTypes::NORMAL && @battle.FE == PBFields::NONE))
	
	typemod *= 4 if (@id == PBMoves::TERRAINKICK) && ((opponent.hasType?(PBTypes::ELECTRIC) && @battle.FE == PBFields::ELECTRICT) || (opponent.hasType?(PBTypes::GRASS) && @battle.FE == PBFields::GRASSYT) || (opponent.hasType?(PBTypes::DARK) && @battle.FE == PBFields::DARKCRYSTALC) || (opponent.hasType?(PBTypes::PSYCHIC) && @battle.FE == PBFields::CHESSB) || (opponent.hasType?(PBTypes::FIRE) && @battle.FE == PBFields::BURNINGF) || (opponent.hasType?(PBTypes::WATER) && @battle.FE == PBFields::SWAMPF) || (opponent.hasType?(PBTypes::POISON) && @battle.FE == PBFields::CORROSIVEF) || (opponent.hasType?(PBTypes::POISON) && @battle.FE == PBFields::CORROSIVEMISTF) || (opponent.hasType?(PBTypes::ICE) && @battle.FE == PBFields::ICYF) || (opponent.hasType?(PBTypes::FIRE) && @battle.FE == PBFields::SUPERHEATEDF) || (opponent.hasType?(PBTypes::STEEL) && @battle.FE == PBFields::FACTORYF) || (opponent.hasType?(PBTypes::POISON) && @battle.FE == PBFields::WASTELAND) || (opponent.hasType?(PBTypes::WATER) && @battle.FE == PBFields::WATERS) || (opponent.hasType?(PBTypes::WATER) && @battle.FE == PBFields::UNDERWATER) || (opponent.hasType?(PBTypes::POISON) && @battle.FE == PBFields::MURKWATERS) || (opponent.hasType?(PBTypes::ICE) && @battle.FE == PBFields::SNOWYM) || (opponent.hasType?(PBTypes::STEEL) && @battle.FE == PBFields::MIRRORA) || (opponent.hasType?(PBTypes::GRASS) && @battle.FE == PBFields::FLOWERGARDENF) || (opponent.hasType?(PBTypes::DARK) && @battle.FE == PBFields::STARLIGHTA) || (opponent.hasType?(PBTypes::PSYCHIC) && @battle.FE == PBFields::PSYCHICT) || (opponent.hasType?(PBTypes::NUCLEAR) && @battle.FE == PBFields::FALLOUT))

	typemod *= 2 if (@id == PBMoves::TERRAINKICK) && ((opponent.hasType?(PBTypes::COSMIC) && @battle.FE == PBFields::GUFIELD) || (opponent.hasType?(PBTypes::COSMIC) && @battle.FE == PBFields::DWORLD) || (opponent.hasType?(PBTypes::FAIRY) && @battle.FE == PBFields::MISTYT) || (opponent.hasType?(PBTypes::FAIRY) && @battle.FE == PBFields::FAIRYTALEF) || (opponent.hasType?(PBTypes::FIGHTING) && @battle.FE == PBFields::BIGTOPA) || (opponent.hasType?(PBTypes::FLYING) && @battle.FE == PBFields::MOUNTAIN) || (opponent.hasType?(PBTypes::GROUND) && @battle.FE == PBFields::DESERTF) || (opponent.hasType?(PBTypes::GROUND) && @battle.FE == PBFields::ASHENB) || (opponent.hasType?(PBTypes::NORMAL) && @battle.FE == PBFields::HOLYF) || (opponent.hasType?(PBTypes::NORMAL) && @battle.FE == PBFields::INVERSEF) || (opponent.hasType?(PBTypes::NORMAL) && @battle.FE == PBFields::INDOORA) || (opponent.hasType?(PBTypes::NORMAL) && @battle.FE == PBFields::INDOORB) || (opponent.hasType?(PBTypes::NORMAL) && @battle.FE == PBFields::INDOORC) || (opponent.hasType?(PBTypes::NORMAL) && @battle.FE == PBFields::CITY) || (opponent.hasType?(PBTypes::NORMAL) && @battle.FE == PBFields::CITYNEW) || (opponent.hasType?(PBTypes::QMARKS) && @battle.FE == PBFields::GLITCHF) || (opponent.hasType?(PBTypes::QMARKS) && @battle.FE == PBFields::NEWW) || (opponent.hasType?(PBTypes::ROCK) && @battle.FE == PBFields::ROCKYF) || (opponent.hasType?(PBTypes::ROCK) && @battle.FE == PBFields::CAVE) || (opponent.hasType?(PBTypes::ROCK) && @battle.FE == PBFields::CRYSTALC) || (opponent.hasType?(PBTypes::BUG)&& @battle.FE == PBFields::FORESTF) || (opponent.hasType?(PBTypes::NORMAL)&& @battle.FE == PBFields::NONE))
...
################################################################################
  def pbAccuracyCheck(attacker,opponent,just_wondering:false) #the just_wondering is for Dragon Darts
    ...
    return true if attacker.ability == PBAbilities::NOGUARD || opponent.ability == PBAbilities::NOGUARD || (attacker.hasWorkingAbility(:FAIRYAURA) && @battle.FE == PBFields::FAIRYTALEF) || attacker.ability == PBAbilities::ALOLANTECHNIQUESA || opponent.ability == PBAbilities::ALOLANTECHNIQUESA || attacker.ability == PBAbilities::SHININGPEARL || attacker.ability == PBAbilities::NIGHOMNIPOTENCE  || attacker.ability == PBAbilities::GODHEAD
	...
	if attacker.ability == PBAbilities::TERAVOLT
      accuracy*=4.0/3
    end
    partner=attacker.pbPartner
    if partner && partner.ability == PBAbilities::TERAVOLT
      accuracy*=4.0/3
    end
	if attacker.ability == PBAbilities::TURBOBLAZE
      accuracy*=4.0/3
    end
    partner=attacker.pbPartner
    if partner && partner.ability == PBAbilities::TURBOBLAZE
      accuracy*=4.0/3
    end
    ...
    if (opponent.ability == PBAbilities::INSTINCT || opponent.ability == PBAbilities::OMNIPOTENT || opponent.ability == PBAbilities::GODHEAD)
      evasion*=1.2
    end
...
################################################################################
  def pbCritRate?(attacker,opponent)
    return -1 if (opponent.ability == PBAbilities::BATTLEARMOR || opponent.ability == PBAbilities::SHELLARMOR || opponent.ability == PBAbilities::SINNOHPOWER) && !(opponent.moldbroken)
  ...
  #---------------------------------------------------------------------------
  # ★ Emotional Resonance, RESOLUTE SWORD
  # For sound-based moves, use the user's higher offensive stat
  # to decide whether damage is Physical or Special.
  #---------------------------------------------------------------------------
  if attacker && ((attacker.ability == PBAbilities::EMOTIONALRESONANCE && self.isSoundBased?) || (attacker.ability == PBAbilities::RESOLUTESWORD && (PBStuff::SLASHMOVE).include?(@id)) || (attacker.ability == PBAbilities::ASONE && attacker.form == 3) || (attacker.ability == PBAbilities::ASONE && attacker.form == 4 && @id == PBMoves::HYPERBEAM))
    calcattack   = attacker.attack
    calcspattack = attacker.spatk
	if attacker.stages[PBStats::ATTACK] != 0
        idx = attacker.stages[PBStats::ATTACK] + 6
        calcattack = (calcattack.to_f * stagemul[idx] / stagediv[idx]).floor
      end

      if attacker.stages[PBStats::SPATK] != 0
        idx = attacker.stages[PBStats::SPATK] + 6
        calcspattack = (calcspattack.to_f * stagemul[idx] / stagediv[idx]).floor
      end
    @category = (calcattack >= calcspattack) ? 0 : 1
  end
  #---------------------------------------------------------------------------
    ...
    if attacker.ability == PBAbilities::TECHNICIAN || attacker.ability == PBAbilities::UMBRALSTRIKER
      if basedmg<=60
	...
	elsif attacker.ability == PBAbilities::BADDREAMS
      damagemult=(damagemult*1.5).round if (opponent.status == PBStatuses::SLEEP)
    elsif attacker.ability == PBAbilities::TOUGHCLAWS && (@flags&0x01)!=0 # Makes direct contact
      damagemult=(damagemult*1.3).round
    elsif (attacker.ability == PBAbilities::IRONFIST || attacker.item == PBItems::PUNCHINGGLOVE || PBAbilities::LIGHTNINGPUNCHER || PBAbilities::SINNOHPOWER) && isPunchingMove?
    ...
    elsif attacker.ability == PBAbilities::UNOVAPOWER
      damagemult=(damagemult*1.5).round if (PBStuff::SLASHMOVE).include?(@id)
	...
	elsif attacker.ability == PBAbilities::TIMETRAVEL && type == PBTypes::PSYCHIC
        damagemult=(damagemult*1.5).round
	elsif attacker.ability == PBAbilities::GODHEAD
        damagemult=(damagemult*1.5).round
	elsif attacker.ability == PBAbilities::ASCENDANCE# && type != PBTypes::QMARKS
        damagemult=(damagemult*1.5).round
	elsif attacker.ability == PBAbilities::ENVIRONMENTALADAPTATION && attacker.form ==21
        damagemult=(damagemult*1.5).round
	elsif (attacker.ability == PBAbilities::FULLMETALBODY || attacker.ability == PBAbilities::NEUROFORCE) && type == PBTypes::FIRE
        damagemult=(damagemult*1.5).round
	elsif (attacker.ability == PBAbilities::SHADOWSHIELD || attacker.ability == PBAbilities::NEUROFORCE) && type == PBTypes::FAIRY
        damagemult=(damagemult*1.5).round
	elsif attacker.species == PBSpecies::LUGIA && type == PBTypes::WATER
        damagemult=(damagemult*1.5).round
	elsif attacker.species == PBSpecies::NECROZMA && type == PBTypes::PSYCHIC
        damagemult=(damagemult*1.5).round
	elsif attacker.ability == PBAbilities::BLAZERUSH && attacker.ability == PBAbilities::HYPERMOMENTUM && @battle.FE == PBFields::GLITCHF
        damagemult=(damagemult*1.5).round
    elsif attacker.ability == PBAbilities::RECKLESS || attacker.ability == PBAbilities::UNOVAPOWER
    ...
    elsif (attacker.ability == PBAbilities::KANTOPOWER)
      if @id == PBMoves::AURASPHERE || @id == PBMoves::DRAGONPULSE || @id == PBMoves::DARKPULSE || @id == PBMoves::WATERPULSE || @id == PBMoves::ORIGINPULSE || @id == PBMoves::LASERPULSE || @id == PBMoves::PROTONBEAM || @id == PBMoves::GROUNDZERO || @id == PBMoves::TERRAINPULSE
        damagemult=(damagemult*1.5).round
      end
      ...
      damagemult=(damagemult*1.3).round
    elsif attacker.ability == PBAbilities::JOHTOPOWER && @addlEffect>0
      damagemult=(damagemult*1.3).round
      ...
      case type
        ...
        when PBTypes::GRASS
          if attacker.item == PBItems::MIRACLESEED || attacker.item == PBItems::MEADOWPLATE || attacker.item == PBItems::ROSEINCENSE
            ...
            damagemult=(damagemult*1.5).round
            attacker.takegem=true
			elsif (attacker.species ==PBSpecies::OGERPON && (attacker.form == 5 || attacker.form == 6 || attacker.form == 7 || attacker.form == 8 || attacker.form == 10))
			damagemult=(damagemult*1.5).round
          end
    ...
    case @battle.FE
      when 5 # Chess Board
        if (PBFields::CHESSMOVES).include?(@id)
          if (opponent.ability == PBAbilities::ADAPTABILITY) || (opponent.ability == PBAbilities::ANTICIPATION) || (opponent.ability == PBAbilities::SYNCHRONIZE) || (opponent.ability == PBAbilities::TELEPATHY) || (opponent.ability == PBAbilities::ORIGINOFTHESPECIES) 
            damagemult=(damagemult*0.5).round
          end
      ...
      when 6 # Big Top
        if ((type == PBTypes::FIGHTING && pbIsPhysical?(type)) || (PBFields::STRIKERMOVES).include?(@id)) # Continental Crush
          ...
          @fieldmessageshown = true
          if attacker.ability == PBAbilities::HUGEPOWER || attacker.ability == PBAbilities::GUTS || attacker.ability == PBAbilities::PUREPOWER || attacker.ability == PBAbilities::SHEERFORCE || attacker.ability == PBAbilities::INFURIATE || attacker.ability == PBAbilities::PUNKROCK || attacker.ability == PBAbilities::JOHTOPOWER
            if striker >=9
              striker = 15
            else
              striker = 14
    ...
    case attacker.ability
    when PBAbilities::GUTS, PBAbilities::PRIDE, PBAbilities::MOLECULARFLUX
	...
	#비크티니 victory star
	if attacker.ability == PBAbilities::VICTORYSTAR || attacker.pbPartner.ability == PBAbilities::VICTORYSTAR
    ...
    if attitemworks
      if attacker.item == PBItems::THICKCLUB 
	  ...
	  elsif attacker.item == PBItems::BERSERKGENE
        atkmult=(atkmult*2.0).round if attacker.pokemon.species == PBSpecies::TAUROS && attacker.pokemon.form ==4
		...
		elsif attacker.item == (PBItems::TEALMASK || PBItems::WELLSPRINGMASK || PBItems::HEARTHFLAMEMASK || PBItems::CORNERSTONEMASK || PBItems:: MAJORAMASK)
        atkmult=(atkmult*1.2).round if attacker.pokemon.species == PBSpecies::OGERPON
		elsif attacker.item == (PBItems::DOUSEDRIVE || PBItems::SHOCKDRIVE || PBItems::BURNDRIVE || PBItems::CHILLDRIVE)
        atkmult=(atkmult*1.2).round if attacker.pokemon.species == PBSpecies::GENESECT
      end
    end
    if @battle.FE !=0
    ...
    if (opponent.ability == PBAbilities::MARVELSCALE || opponent.ability == PBAbilities::MOLECULARFLUX) && pbIsPhysical?(type) &&
    ...
    if ((opponent.ability == PBAbilities::GRASSPELT) || (opponent.ability == PBAbilities::JUNGLEGUARDIAN) || (opponent.ability == PBAbilities::GRACEFULBLOOM)) && pbIsPhysical?(type) &&
    (@battle.FE == PBFields::GRASSYT || @battle.FE == PBFields::FORESTF) #not affected by mold breaker?
      defmult=(defmult*1.5).round
    end
	if ((opponent.ability == PBAbilities::GRACEFULBLOOM)) && pbIsPhysical?(type) && (@battle.FE == PBFields::FLOWERGARDENF) #not affected by mold breaker?
      defmult=(defmult*1.5).round
	end
    ...
    if (opponent.ability == PBAbilities::ICESCALES || opponent.ability == PBAbilities::MAGICCHILL) && pbIsSpecial?(type) && !(opponent.moldbroken)
      defmult=(defmult*2).round
    end
	...
	#비크티니 VICTORYSTAR
	if opponent.ability == PBAbilities::VICTORYSTAR
        defmult=(defmult*1.1).round
    end
    if opponent.pbPartner.ability == PBAbilities::VICTORYSTAR
        defmult=(defmult*1.1).round
    end
    ...
    if opponent.hasWorkingItem(:EVIOLITE) && @battle.FE != PBFields::GLITCHF
      ...
      if opponent.item == PBItems::EEVIUMZ2 && opponent.pokemon.species == (PBSpecies::EEVEE || PBSpecies::EEVEON) && @battle.FE != PBFields::GLITCHF
        defmult=(defmult*1.5).round
    end
    ...
    case @battle.pbWeather
      when PBWeather::SUNNYDAY
        if @battle.state.effects[PBEffects::HarshSunlight] && type == PBTypes::WATER
          ...
          return 0 if ((@id!=PBMoves::HYDROSTEAM) || (@id!=PBMoves::STEAMERUPTION))
        end
    ...
    case @battle.pbWeather
      when PBWeather::SUNNYDAY
        if (type == PBTypes::FIRE || @id == PBMoves::HYDROSTEAM || @id ==PBMoves::STEAMERUPTION )
    ...
    if attacker.status==PBStatuses::BURN && pbIsPhysical?(type) &&
       attacker.ability != PBAbilities::GUTS && attacker.ability != PBAbilities::PRIDE && attacker.ability != PBAbilities::MOLECULARFLUX && attacker.ability != PBAbilities::SINISTERBLAZE && @id != PBMoves::FACADE
      damage=(damage*0.5).round
    end
    ...
    if (((opponent.ability == PBAbilities::MULTISCALE || opponent.ability == PBAbilities::ROYALPINK) && !(opponent.moldbroken)) || opponent.ability == PBAbilities::SHADOWSHIELD) && opponent.hp==opponent.totalhp
      finaldamagemult=(finaldamagemult*0.5).round
    end
    ...
    if (((opponent.ability == PBAbilities::SOLIDROCK || opponent.ability == PBAbilities::FILTER) && !opponent.moldbroken) ||
       opponent.ability == PBAbilities::PRISMARMOR || opponent.ability == PBAbilities::NEUROFORCE) && opponent.damagestate.typemod>4
      finaldamagemult=(finaldamagemult*0.75).round
    end
  ...
  def pbReduceHPDamage(damage,attacker,opponent)
    ...
    if opponent.effects[PBEffects::Substitute]>0 && (!attacker || attacker.index!=opponent.index) &&
     attacker.ability != PBAbilities::INFILTRATOR && !isSoundBased? && !(attacker.ability == PBAbilities::INTREPIDSWORD && @id == PBMoves::BEHEMOTHBLADESTELLAR) &&
     @id!=PBMoves::SPECTRALTHIEF &&  @id!=PBMoves::HYPERSPACEHOLE &&  @id!=PBMoves::HYPERSPACEFURY #spectral thief/ hyperspace hole/ hyperspace fury
      ...
      if damage>=opponent.hp
        ...
        if @function==0xE9 # False Swipe
          ...
          damage=damage-1
		elsif (opponent.ability == PBAbilities::GRACEFULBLOOM && opponent.form ==1 ) && !opponent.moldbroken
          opponent.damagestate.sturdy=true
          damage=damage-1
...
################################################################################
  def pbEffectMessages(attacker,opponent,ignoretype=false)
    ...
    if opponent.damagestate.endured
      ...
      @battle.pbDisplay(_INTL("{1} hung on the edge of the board!",opponent.pbThis))
		...
		if opponent.species == PBSpecies::SHAYMIN
			@battle.pbDisplay(_INTL("{1} returned to its original form!",opponent.pbThis))
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @id == PBMoves::GUARDIANOFALOLA
	  ...
	  /
	elsif (@id == PBMoves::SEARINGSUNRAZESMASH || @id == PBMoves::MENACINGMOONRAZEMAELSTROM)
	...
	if (@id == PBMoves::SEARINGSUNRAZESMASH || @id == PBMoves::MENACINGMOONRAZEMAELSTROM)
		if attacker.pbCanIncreaseStatStage?(PBStats::SPEED,abilitymessage:false)
		attacker.pbIncreaseStat(PBStats::SPEED,2,abilitymessage:false)
		end
	end
	...
	if @id ==PBMoves::SOULSTEALING7STARSTRIKE
		totalboost = 0
    if pbTypeModifier(@type,attacker,opponent) != 0 || @battle.FE == PBFields::INVERSEF
      for i in [PBStats::ATTACK,PBStats::DEFENSE,PBStats::SPEED,
        ...
        if opponent.stages[i]>0
		  ...
		  oppboost *= -1 if attacker.ability == PBAbilities::UNOVAPOWER
  ...
  def positivePriority?(attacker)
	...
	pri += 1 if @battle.FE == PBFields::FORESTF && @id==PBMoves::JUNGLEGLIDE
	...
	pri += 1 if attacker.ability == PBAbilities::BLAZERUSH && @type==PBTypes::FIRE
	pri += 1 if attacker.ability == PBAbilities::BRILLIANTDIAMOND || attacker.ability == PBAbilities::NIGHOMNIPOTENCE	
    return pri > 0
  end
