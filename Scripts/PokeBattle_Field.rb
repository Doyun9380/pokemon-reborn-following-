...
class PokeBattle_Battle
  ...
  def mistExplosion
    if !pbCheckGlobalAbility(:DAMP) && !pbCheckGlobalAbility(:HOENNPOWER)
  ...
  def caveCollapse
    if @field.counter == 0
      ...
      for i in @battlers
        ...
        quakedrop /= 2 if i.ability == PBAbilities::SHELLARMOR || i.ability == PBAbilities::BATTLEARMOR || i.ability == PBAbilities::SINNOHPOWER
  ...
  def mirrorShatter
    ...
    for i in @battlers
      ...
      next if shatter == 0
      next if PBStuff::TWOTURNMOVE.include?(i.effects[PBEffects::TwoTurnAttack])
      next if i.ability == PBAbilities::SHELLARMOR || i.ability == PBAbilities::BATTLEARMOR || i.ability == PBAbilities::SINNOHPOWER
      next if i.pbOwnSide.effects[PBEffects::WideGuard]
      next if i.effects[PBEffects::SkyDrop] || i.effects[PBEffects::Commander]
...
class PokeBattle_Battler
  def burningFieldPassiveDamage?
    ...
    return false if [PBAbilities::FLAREBOOST,PBAbilities::MAGMAARMOR,PBAbilities::FLAMEBODY,PBAbilities::FLASHFIRE,PBAbilities::WELLBAKEDBODY,PBAbilities::ABYSSALFURNACE,PBAbilities::JOHTOPOWER].include?(@ability)
    return false if [PBAbilities::WATERVEIL,PBAbilities::MAGICGUARD,PBAbilities::HEATPROOF,PBAbilities::WATERBUBBLE,PBAbilities::THERMALEXCHANGE, PBAbilities::ORICHALCUMPULSE].include?(@ability)
  ...
  def underwaterFieldPassiveDamamge?
    ...
    return false if @ability == PBAbilities::SWIFTSWIM || @ability == PBAbilities::MAGICGUARD || @ability == PBAbilities::WATERABSORB || @ability == PBAbilities::EMOTIONALTIDE
  ...
  def murkyWaterSurfacePassiveDamage?
    ...
    return false if [PBAbilities::POISONHEAL, PBAbilities::MAGICGUARD, PBAbilities::WONDERGUARD, PBAbilities::TOXICBOOST, PBAbilities::IMMUNITY, PBAbilities::RENEGADEPLATINUM, PBAbilities::NIGHOMNIPOTENCE].include?(@ability)
    return true
  end
  
  def falloutPassiveDamage? # Uranium
    return false if pbHasType?(:STEEL) || pbHasType?(:NUCLEAR) || pbHasType?(:COSMIC) || pbHasType?(:STELLAR)
