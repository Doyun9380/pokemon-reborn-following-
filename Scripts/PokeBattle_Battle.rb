...
################################################################################
  ...
  def pbWeather
    for i in 0...4
      if @battlers[i].ability == PBAbilities::CLOUDNINE || @battlers[i].ability == PBAbilities::AIRLOCK || @field.effect == PBFields::UNDERWATER || @battlers[i].ability == PBAbilities::TERAFORMZERO || @battlers[i].ability == PBAbilities::SHININGPEARL || @battlers[i].ability == PBAbilities::NIGHOMNIPOTENCE || @battlers[i].ability == PBAbilities::GODHEAD  
        return 0
      end
    end
    return @weather
  ...
  def seedCheck
    for battler in pbPriority
      ...
      seeddata[:stats].each_pair {|stat,statval|
        statval *= -1 if battler.ability == PBAbilities::CONTRARY || battler.ability == PBAbilities::ALOLANTECHNIQUESB || battler.ability == PBAbilities::UNOVAPOWER
  ...
  def pbIsUnlosableItem(pkmn,item)
    ...
    if (pkmn.species == PBSpecies::ARCEUS)
      if [PBItems::FISTPLATE,   PBItems::SKYPLATE,    PBItems::TOXICPLATE,  PBItems::EARTHPLATE,  PBItems::STONEPLATE,
          ...
          PBItems::PIXIEPLATE,  PBItems::DREADPLATE,  PBItems::ATOMICPLATE, PBItems::COSMICPLATE, PBItems::LEGENDPLATE].include?(item)
        return true
      end
    end
	...
	return true if (pkmn.species == PBSpecies::GRENINJA) && (item == PBItems::RYUICHIMONJI)
    return false
  end
  ...
  def pbPriority(ignorequickclaw = true,megacalc = false)
    ...
    for i in 0..3
      ...
      if @choices[i][0] == 1 # Is a move
        ...
        pri += 1 if @battlers[i].ability == PBAbilities::BRILLIANTDIAMOND || @battlers[i].ability == PBAbilities::NIGHOMNIPOTENCE	
		pri += 3 if @battlers[i].ability == PBAbilities::TRIAGE && (PBStuff::HEALFUNCTIONS).include?(@choices[i][2].function)
		pri += 1 if @battlers[i].ability == PBAbilities::BLAZERUSH && @choices[i][2].type==PBTypes::FIRE
		pri += 3 if @battlers[i].ability == PBAbilities::INTREPIDSWORD && @choices[i][2].type==PBTypes::STELLAR
		pri += 3 if @battlers[i].ability == PBAbilities::DAUNTLESSSHIELD && @choices[i][2].type==PBTypes::STELLAR
      end
...
################################################################################
  ...
  def pbCanSwitch?(idxPokemon,pkmnidxTo,showMessages,ai_phase=false,running: false)
    ...
    unless thispkmn.ability == (PBAbilities::SHADOWTAG || PBAbilities::BLACKHOLE || PBAbilities::UMBRALSTRIKER)
      opp=opp1 if opp1.ability == (PBAbilities::SHADOWTAG || PBAbilities::BLACKHOLE || PBAbilities::UMBRALSTRIKER)
      opp=opp2 if opp2.ability == (PBAbilities::SHADOWTAG || PBAbilities::BLACKHOLE || PBAbilities::UMBRALSTRIKER)
    end
	unless thispkmn.status == (PBStatuses::SLEEP)
      opp=opp1 if opp1.ability == (PBAbilities::BADDREAMS)
      opp=opp2 if opp2.ability == (PBAbilities::BADDREAMS)
    end
    if opp
  ...
  # Giga Form 해제 처리
  # -------------------------------
  if @battlers[index].species == PBSpecies::CINDERACE && @battlers[index].form == 2
    @battlers[index].form = 0
    @battlers[index].effects[PBEffects::GigaFormTurns] = 0
    @battlers[index].pbUpdate(true)
    pbDisplay(_INTL("{1}'s Giga form faded upon switching out!", @battlers[index].pbThis))
  end
  # -------------------------------
...
################################################################################
  ...
  def pbRun(idxPokemon,duringBattle=false)
    ...
    if @opponent
      if Input.press?(Input::CTRL)				# if $DEBUG && Input.press?(Input::CTRL)		
...
################################################################################
  ...
  def pbMegaEvolve(index)
    ...
    elsif [PBItems::TEALMASK, PBItems::WELLSPRINGMASK, PBItems::HEARTHFLAMEMASK, PBItems::CORNERSTONEMASK, PBItems::MAJORAMASK].include?(@battlers[index].item)
    ...
    if !onlyabilities
      ...
      if pkmn.pbOwnSide.effects[PBEffects::Spikes]>0
        if !pkmn.isAirborne? && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS)
          if (pkmn.ability != PBAbilities::MAGICGUARD && pkmn.ability != PBAbilities::SHININGPEARL && pkmn.ability != PBAbilities::NIGHOMNIPOTENCE && pkmn.ability != PBAbilities::GODHEAD) 
      ...
      if pkmn.pbOwnSide.effects[PBEffects::StealthRock]
        if (pkmn.ability != PBAbilities::MAGICGUARD || pkmn.ability != PBAbilities::MOUNTAINEER || pkmn.ability != PBAbilities::OMNIPOTENT) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS) && @field.effect != PBFields::WASTELAND && pkmn.ability != PBAbilities::SHININGPEARL && pkmn.ability != PBAbilities::NIGHOMNIPOTENCE && pkmn.ability != PBAbilities::GODHEAD  
          atype = PBTypes::ROCK
      ...
      if pkmn.pbOwnSide.effects[PBEffects::Decrystals]
        if pkmn.ability != PBAbilities::MAGICGUARD && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS) && @field.effect != PBFields::WASTELAND && pkmn.ability !=PBAbilities::SHININGPEARL && pkmn.ability !=PBAbilities::NIGHOMNIPOTENCE  && pkmn.ability != PBAbilities::GODHEAD 
      ...
      if @field.effect == PBFields::CORROSIVEF
        if !(pkmn.ability == PBAbilities::MAGICGUARD || pkmn.ability == PBAbilities::POISONHEAL || pkmn.ability == PBAbilities::IMMUNITY || pkmn.ability == PBAbilities::WONDERGUARD || pkmn.ability == PBAbilities::MULTICORE || pkmn.ability == PBAbilities::RENEGADEPLATINUM ||
            pkmn.ability == PBAbilities::TOXICBOOST) && !pkmn.isAirborne? && !pkmn.pbHasType?(:POISON) && !pkmn.pbHasType?(:STEEL) && pkmn.ability !=PBAbilities::SHININGPEARL && pkmn.ability !=PBAbilities::NIGHOMNIPOTENCE  && pkmn.ability != PBAbilities::GODHEAD 
      ...
      if pkmn.pbOwnSide.effects[PBEffects::StickyWeb]
        if !pkmn.isAirborne? && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS) && pkmn.ability !=PBAbilities::SHININGPEARL && pkmn.ability !=PBAbilities::NIGHOMNIPOTENCE  && pkmn.ability != PBAbilities::GODHEAD 
      ...
      if pkmn.pbOwnSide.effects[PBEffects::ToxicSpikes]>0 && !pkmn.isAirborne?
        ...
        elsif pkmn.pbCanPoisonSpikes?(true) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS) && pkmn.ability !=PBAbilities::SHININGPEARL && pkmn.ability !=PBAbilities::NIGHOMNIPOTENCE  && pkmn.ability != PBAbilities::GODHEAD 
...
################################################################################
 ...
 # G-Max Cinderace 에이스번 처리 변신한 턴에도 공격 바꾸기
    for i in priority
      next if @choices[i.index][0] != 1   # 1 = 기술 선택
      battler = i
      move = @choices[i.index][2]
      next if !move || move==0
		
	  if battler.species == PBSpecies::TERAPAGOS && battler.form == 2		#테라클러스터
        if move.id == PBMoves::TERASTARSTORM
          base  = PBMove.new(PBMoves::TERASTARSTORM2)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
      end
	  
	  if battler.species == PBSpecies::ZYGARDE && battler.form == 3		#니힐레이저
        if move.id == PBMoves::COREENFORCER
          base  = PBMove.new(PBMoves::NIHILLIGHT)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
      ...
      # Cinderace가 G-Max 상태이고, Pyro Ball을 선택했다면 강제로 교체
      if battler.species == PBSpecies::CINDERACE && battler.form == 2
        if move.id == PBMoves::PYROBALL
          # G-Max Fireball로 교체
          base  = PBMove.new(PBMoves::GMXFIREBALL)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
          pbDisplay(_INTL("{1}'s move turned into G-Max Fireball!", battler.pbThis))
        end
		if move.id == PBMoves::HIJUMPKICK
          # Max Knuckle로 교체
          base  = PBMove.new(PBMoves::MAXKNUCKLE)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
          pbDisplay(_INTL("{1}'s move turned into Max Knuckle!", battler.pbThis))
        end
		if move.id == PBMoves::BOUNCE
          # Max Airstream로 교체
          base  = PBMove.new(PBMoves::MAXAIRSTREAM)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
          pbDisplay(_INTL("{1}'s move turned into Max Airstream!", battler.pbThis))
        end
		if move.id == PBMoves::IRONHEAD
          # Max Steelspike로 교체
          base  = PBMove.new(PBMoves::MAXSTEELSPIKE)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
          pbDisplay(_INTL("{1}'s move turned into Max Steelspike!", battler.pbThis))
        end
      end
    end


# G-Max Cinderace 에이스번 변신 풀린 턴에 공격 바꾸기
    for i in priority
      next if @choices[i.index][0] != 1   # 1 = 기술 선택
      battler = i
      move = @choices[i.index][2]
      ...
      if battler.species == PBSpecies::TERAPAGOS && battler.form != 2
	    if move.id == PBMoves::TERASTARSTORM2
          base  = PBMove.new(PBMoves::TERASTARSTORM)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
	  ...
	  if battler.species == PBSpecies::ZYGARDE && battler.form != 3
	    if move.id == PBMoves::NIHILLIGHT
          base  = PBMove.new(PBMoves::COREENFORCER)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
      ...
      if battler.species == PBSpecies::CINDERACE && battler.form == 0
        if move.id == PBMoves::GMXFIREBALL

          base  = PBMove.new(PBMoves::PYROBALL)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
		if move.id == PBMoves::MAXKNUCKLE
          base  = PBMove.new(PBMoves::HIJUMPKICK)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
		if move.id == PBMoves::MAXAIRSTREAM
          base  = PBMove.new(PBMoves::BOUNCE)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
		if move.id == PBMoves::MAXSTEELSPIKE
          base  = PBMove.new(PBMoves::IRONHEAD)
          newmv = PokeBattle_Move.pbFromPBMove(self, base, battler)
          newmv.pp = move.pp   # 기존 PP 사용 (비율까지 맞추고 싶으면 더 계산 가능)
          @choices[i.index][2] = newmv
        end
...
################################################################################
  def pbEndOfRoundPhase
    ...
    for i in priority
      ...
      case @field.effect
        ...
        when PBFields::BURNINGF # Burning Field
          next if i.hp<=0
          if !i.isAirborne?
            if ((i.ability == PBAbilities::FLASHFIRE) || (i.ability == PBAbilities::ABYSSALFURNACE) || (i.ability == PBAbilities::JOHTOPOWER))
        ...
        when PBFields::FALLOUT # Uranium
          ...
          if i.falloutPassiveDamage?
            ...
            if eff>0 && i.type1 != PBTypes::STELLAR && i.type2 != PBTypes::STELLAR && i.type1 != PBTypes::QMARKS && i.type2 != PBTypes::QMARKS
              @scene.pbDamageAnimation(i,0)
        ...
        when PBFields::SHORTCIRCUITF # Shortcircuit Field
          next if i.hp<=0
          if ((i.ability == PBAbilities::VOLTABSORB) || (i.ability == PBAbilities::LIGHTNINGPUNCHER) ) && i.effects[PBEffects::HealBlock]==0
            hpgain=(i.totalhp/16.0).floor
            hpgain=i.pbRecoverHP(hpgain,true)
          ...
          if pbWeather==PBWeather::SUNNYDAY
            ...
            for i in priority
              ...
              if i.ability == PBAbilities::SOLARPOWER
                ...
                if !i.pbHasType?(:FIRE) && !i.pbHasType?(:DRAGON) && i.ability != PBAbilities::FLASHFIRE && i.ability != PBAbilities::ABYSSALFURNACE && i.ability != PBAbilities::JOHTOPOWER && i.ability != PBAbilities::UBERSTRAT && i.ability != PBAbilities::MULTICORE && i.ability != PBAbilities::MAGICGUARD && ![0xCA,0xCB].include?($cache.pkmn_move[i.effects[PBEffects::TwoTurnAttack]][PBMoveData::FUNCTION]) # Dig, Dive
        ...
        if @weatherduration==0
          ...
          if pbWeather==PBWeather::RAINDANCE && @state.effects[PBEffects::Storm]
            ...
            for i in priority
              ...
              i.ability == PBAbilities::LIGHTNINGROD || i.ability == PBAbilities::MAGICGUARD || i.ability == PBAbilities::HOENNPOWER ||i.ability == PBAbilities::LIGHTNINGPUNCHER) &&
        ...
        if @weatherduration==0
          ...
          if pbWeather==PBWeather::NUCLEARFALLOUT
            ...
            for i in priority
              next if i.isFainted?
              eff = PBTypes.getCombinedEffectiveness(PBTypes::NUCLEAR, i.type1, i.type2)
              if !i.pbHasType?(:NUCLEAR) && !i.pbHasType?(:STEEL) && !i.pbHasType?(:COSMIC) && !i.pbHasType?(:STELLAR) && !(i.ability == PBAbilities::MAGICGUARD || i.ability == PBAbilities::BULLETPROOF || i.ability == PBAbilities::LEADSKIN) &&
    ...
    for i in battlers   # not priority
      ...
      if (i.isFainted? || move.pbAccuracyCheck(moveuser,i) && !((i.ability == PBAbilities::WONDERGUARD || i.ability == PBAbilities::RENEGADEPLATINUM || i.ability == PBAbilities::NIGHOMNIPOTENCE ) && typemod<=4)) && !twoturninvul
        i.damagestate.reset
        damage = nil
	  ...
	  elsif (i.ability == PBAbilities::RENEGADEPLATINUM || i.ability == PBAbilities::NIGHOMNIPOTENCE ) && typemod<=4 && !twoturninvul
        pbDisplay(_INTL("{1} avoided damage with its power!",i.pbThis))
      else
    ...
    for i in priority
	  ...
	  if i.ability == PBAbilities::ORIGINOFTHESPECIES && i.effects[PBEffects::HealBlock]==0
        hpgain=i.pbRecoverHP((i.totalhp/16.0).floor,true)
        pbDisplay(_INTL("{1} regenerated through its genetic power!",i.pbThis)) if hpgain>0
      end
	  if i.ability == PBAbilities::FAIRYAURA && i.effects[PBEffects::HealBlock]==0
	    if (i.pbOpposing1.hasWorkingAbility(:AURABREAK) || i.pbOpposing2.hasWorkingAbility(:AURABREAK) || i.pbPartner.hasWorkingAbility(:AURABREAK))
			hploss=i.pbReduceHP((i.totalhp/16.0).floor,true)
			pbDisplay(_INTL("{1} is losing its life by the reversed aura!",i.pbThis)) if hploss>0
		else
			hpgain=i.pbRecoverHP((i.totalhp/16.0).floor,true)
			pbDisplay(_INTL("{1} recovered with the power of the aura!",i.pbThis)) if hpgain>0
		end
      end
	  if i.pbPartner.ability == PBAbilities::FAIRYAURA && i.effects[PBEffects::HealBlock]==0
	    if (i.pbOpposing1.hasWorkingAbility(:AURABREAK) || i.pbOpposing2.hasWorkingAbility(:AURABREAK))
			hploss=i.pbReduceHP((i.totalhp/16.0).floor,true)
			pbDisplay(_INTL("{1} is losing its life by the reversed aura!",i.pbThis)) if hploss>0
		else
			hpgain=i.pbRecoverHP((i.totalhp/16.0).floor,true)
	 ...
	 # if (i.ability == PBAbilities::KANTOPOWER || i.ability == PBAbilities::JOHTOPOWER || i.ability == PBAbilities::HOENNPOWER || i.ability == PBAbilities::SINNOHPOWER || i.ability == PBAbilities::UNOVAPOWER) && i.item != PBItems::PULSEHOLD
	#		hploss=i.pbReduceHP((i.totalhp/8.0).floor,true)
	#		pbDisplay(_INTL("{1} is losing its life by its unstable DNA!",i.pbThis)) if hploss>0
    #  end
	  if (i.species == PBSpecies::BULBMANTLE || i.species == PBSpecies::IVYMELORTLE || i.species == PBSpecies::VENUSTOIZARD || i.species == PBSpecies::TORITAQUIL || i.species == PBSpecies::QUILEEFANAW || i.species == PBSpecies::MEGALAGASION || i.species == PBSpecies::TORIPCKO || i.species == PBSpecies::GROMARSHKEN || i.species == PBSpecies::SWAMPTILIKEN || i.species == PBSpecies::TURCHARPLUP || i.species == PBSpecies::PRINFERNOTLE || i.species == PBSpecies::TORTERNEON || i.species == PBSpecies::TESHAVY || i.species == PBSpecies::PIGVINAUT || i.species == PBSpecies::EMPEROTT || i.species == PBSpecies::VAPFLOLTEON) && i.item != PBItems::PULSEHOLD
			hploss=i.pbReduceHP((i.totalhp/8.0).floor,true)
			pbDisplay(_INTL("{1} is losing its life by its unstable DNA!",i.pbThis)) if hploss>0
      end
      ...
      if i.ability == PBAbilities::GODHEAD && i.effects[PBEffects::HealBlock]==0
        hpgain=i.pbRecoverHP((i.totalhp/16.0).floor,true)
        pbDisplay(_INTL("{1} regenerated through GODHEAD!",i.pbThis)) if hpgain>0
      end
      # Rain Dish
	  ...
	  if i.ability == PBAbilities::EMOTIONALTIDE && (pbWeather==PBWeather::RAINDANCE && !i.hasWorkingItem(:UTILITYUMBRELLA)) && i.effects[PBEffects::HealBlock]==0
        hpgain=i.pbRecoverHP((i.totalhp/16.0).floor,true)
        pbDisplay(_INTL("{1}'s Emotional Tide restored its HP a little!",i.pbThis)) if hpgain>0
      end
      # Dry Skin
    ...
    for i in priority
      ...
      # Hydration
      if (i.ability == PBAbilities::HYDRATION || i.ability == PBAbilities::EMOTIONALTIDE) && ((pbWeather==PBWeather::RAINDANCE && !i.hasWorkingItem(:UTILITYUMBRELLA)) || @field.effect == PBFields::WATERS || @field.effect == PBFields::UNDERWATER)
        if i.status>0
    ...
    for i in priority
      ...
      # Burn
      if i.status==PBStatuses::BURN && i.ability != PBAbilities::MAGICGUARD && i.ability != PBAbilities::SINISTERBLAZE
        i.pbContinueStatus
  ...
  # Giga Form 턴 카운트 체크
  # ----------------------
  if i.species == PBSpecies::CINDERACE && i.form == 2
    if i.effects[PBEffects::GigaFormTurns] == 0
      i.effects[PBEffects::GigaFormTurns] = 2   # 변신 직후 3턴 카운터 설정
    else
      i.effects[PBEffects::GigaFormTurns] -= 1
    end

    if i.effects[PBEffects::GigaFormTurns] <= 0
      i.form = 0
      i.effects[PBEffects::GigaFormTurns] = 0
      i.pbUpdate(true)
	  @scene.pbCommonAnimation("UltraBurst",i,nil)
      @scene.pbChangePokemon(i, i.pokemon)
      pbDisplay(_INTL("{1} returned to its original form!", i.pbThis))
    else
      pbDisplay(_INTL("{1}'s Giga form will wear off in {2} turn(s)!", i.pbThis, i.effects[PBEffects::GigaFormTurns]))
    end
  end
    ...
    if @field.effect == PBFields::WASTELAND
      for i in priority
        ...
        if i.pbOwnSide.effects[PBEffects::StickyWeb]
          ...
          for mon in [i, i.pbPartner]
            next if mon.isFainted? && !PBStuff::TWOTURNMOVE.include?(mon.effects[PBEffects::TwoTurnAttack])
            if (mon.ability == PBAbilities::CONTRARY || mon.ability == PBAbilities::ALOLANTECHNIQUESB || mon.ability == PBAbilities::UNOVAPOWER || mon.ability == PBAbilities::REVERSALIZE) && !mon.pbTooHigh?(PBStats::SPEED)
    ...
    for i in priority
      ...
      if i.turncount>0 && (i.ability == PBAbilities::SPEEDBOOST || i.ability == PBAbilities::HOENNPOWER || (@field.effect == PBFields::ELECTRICT && i.ability == PBAbilities::MOTORDRIVE))
        if !i.pbTooHigh?(PBStats::SPEED)
          i.pbIncreaseStatBasic(PBStats::SPEED,1)
          pbCommonAnimation("StatUp",i,nil)
      ...
      if @field.effect == PBFields::SWAMPF && !([PBAbilities::WHITESMOKE, PBAbilities::CLEARBODY, PBAbilities::FULLMETALBODY, PBAbilities::FLOWERPOWER, PBAbilities::QUICKFEET, PBAbilities::SWIFTSWIM, PBAbilities::EMOTIONALTIDE, PBAbilities::SOULHEART, PBAbilities::ROYALPINK].include?(i.ability))
        if !i.isAirborne?
          if !i.pbTooLow?(PBStats::SPEED)
            contcheck = (i.ability == PBAbilities::CONTRARY || i.ability == PBAbilities::ALOLANTECHNIQUESB || i.ability == PBAbilities::UNOVAPOWER || i.ability == PBAbilities::REVERSALIZE)
      ...
      i.ability != PBAbilities::WONDERGUARD && i.ability != PBAbilities::RENEGADEPLATINUM && i.ability != PBAbilities::NIGHOMNIPOTENCE  && i.ability != PBAbilities::PLOTARMOR && !i.isAirborne? && !i.pbHasType?(:STEEL) && !i.pbHasType?(:POISON) && @field.effect == PBFields::CORROSIVEF
        hploss=i.pbReduceHP((i.totalhp/16.0).floor,true)
      ...
      if (i.ability == PBAbilities::THERMALEXCHANGE || i.ability == PBAbilities::ORICHALCUMPULSE)
        if [PBFields::BURNINGF,PBFields::SUPERHEATEDF,PBFields::DRAGONSD].include?(@field.effect)
      ...
      if (i.pbOpposing1.hasWorkingAbility(:DARKAURA) && i.pbOpposing1.species == PBSpecies::YVELTAL) || (i.pbOpposing2.hasWorkingAbility(:DARKAURA) && i.pbOpposing2.species == PBSpecies::YVELTAL)
        if (i.pbOpposing1.hasWorkingAbility(:AURABREAK) || i.pbOpposing2.hasWorkingAbility(:AURABREAK) || i.pbPartner.hasWorkingAbility(:AURABREAK) || i.ability == PBAbilities::AURABREAK)
			hpgain=i.pbRecoverHP((i.totalhp/16.0).floor,true)
			pbDisplay(_INTL("{1} recovered by the reversed aura!",i.pbThis)) if hpgain>0
		else
		  hploss=i.pbReduceHP((i.totalhp/16.0).floor,true)
          pbDisplay(_INTL("{1} is losing its life by the aura!",i.pbThis)) if hploss>0
		end
      end
	  
	  if (i.pbOpposing1.hasWorkingAbility(:SINISTERBLAZE) && i.pbOpposing1.status==PBStatuses::BURN) || (i.pbOpposing2.hasWorkingAbility(:SINISTERBLAZE) && i.pbOpposing2.status==PBStatuses::BURN)       
		  hploss=i.pbReduceHP((i.totalhp/16.0).floor,true)
          pbDisplay(_INTL("{1} is burned by the foe's Sinister Blaze!",i.pbThis)) if hploss>0
      end
...
################################################################################
  def pbEndOfBattle(canlose=false)
    ...
    for i in @party1
	  ...
	  i.form=0 if i.species == PBSpecies::TERAPAGOS && i.form != 0
    end
    for i in $Trainer.party
