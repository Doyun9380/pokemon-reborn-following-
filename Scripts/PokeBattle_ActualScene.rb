...
class FightMenuButtons < BitmapSprite
  ...
  def initialize(index=0,moves=nil,viewport=nil)
    ...
    @ultraburstbitmap=AnimatedBitmap.new(_INTL("Graphics/Pictures/Battle/GMXEvo"))
	...
	@gmxbitmap=AnimatedBitmap.new(_INTL("Graphics/Pictures/Battle/GMXEvo"))
  ...
  def refresh(index,moves,megaButton,zButton,ultraButton)
    ...
    if ultraButton>0
		#if @battler && @battler.hasItem?(:MAXMUSHROOMS) # GMX (거다이맥스)
			self.bitmap.blt(146,0,@gmxbitmap.bitmap,Rect.new(0,(ultraButton-1)*46,96,46))
		#else
...
class PokemonDataBox < SpriteWrapper
  ...
  def refresh (loopstop=false)
	...
	elsif @battler.pokemon.species == PBSpecies::PALDIATINA && @battler.pokemon.form == 1
      imagepos.push(["Graphics/Pictures/Battle/battlePulseEvoBox.png",@spritebaseX+megaX-10,@spritebaseY+megaY,0,0,-1,-1])
	elsif @battler.pokemon.species == PBSpecies::ZACIAZENTUS && @battler.pokemon.isShiny? == true
