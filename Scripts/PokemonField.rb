...
def pbFieldDamage
  ...
  for i in $Trainer.party
    if i.hp>0 && !i.isEgg? && !(i.ability == PBAbilities::MAGICGUARD) && 
        ...
        i.ability == PBAbilities::MOTORDRIVE || i.ability == PBAbilities::LIGHTNINGROD || i.ability == PBAbilities::LIGHTNINGPUNCHER || i.ability == PBAbilities::HOENNPOWER ||
      ...
      i.hp=1 if i.hp==0
      pbCheckAllFainted()
    elsif ((i.ability == PBAbilities::VOLTABSORB) || (i.ability == PBAbilities::LIGHTNINGPUNCHER))
...
def pbCreateFollowingPokemon(spriteName)
  ...
  x = [[x, 0].max, $game_map.width-1].min
  y = [[y, 0].max, $game_map.height-1].min
